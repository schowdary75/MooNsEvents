...
async function enqueue(type: MaintenanceJob['type']) {
  ...
  for (const tenant of tenants) {
    const key = `moonsevents:tenant:${tenant.id}:cron:${type}:${scheduledAt.toISOString()}`;
  ...
  if (env.legacyRoutingEnabled) {
    const key = `moonsevents:legacy:cron:${type}:${scheduledAt.toISOString()}`;
    const acquired = await redis.set(key, process.pid.toString(), 'EX', 3_600, 'NX');
    if (acquired) {
      await maintenanceQueue.add(
        type,
...
export function startScheduler() {
  ...
  const schedules: Array<[string, string, MaintenanceJob['type']]> = [
    ...
    [env.cron.eventAutomation, 'events automation', 'events-automation'],
