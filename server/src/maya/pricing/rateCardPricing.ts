...
/** Whether a rate card may be used for this party size and events date. */
export function assessRateCard(
  ...
  if (rc.validFrom && eventDate.getTime() < rc.validFrom.getTime())
    reasons.push('rate not yet valid');
  if (rc.validTo && eventDate.getTime() > rc.validTo.getTime()) reasons.push('rate expired');
...
export function priceLine(
  ...
  const { usable, reasons } = assessRateCard(rc, pax, eventDate);
