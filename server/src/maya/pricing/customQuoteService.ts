...
async function findRateCard(
  ...
  const rc = await deps.prisma.catalog_rate_cards.findFirst({
    where: {
      ...
      is_active: true,
      AND: [
        { OR: [{ valid_from: null }, { valid_from: { lte: eventDate } }] },
        { OR: [{ valid_to: null }, { valid_to: { gte: eventDate } }] },
      ],
    },
...
export async function buildCustomQuote(
  ...
  for (const item of req.items) {
    const rc = await findRateCard(deps, item.catalogType, item.catalogId, req.eventDate);
    lines.push(priceLine(item, rc, req.clients, req.eventDate));
  }
