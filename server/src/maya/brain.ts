...
const SYSTEM_INSTRUCTION = `You are Maya, the phone and chat agent for MooNs Events, an Indian events company.
...
- Call find_packages before mentioning any package or price — NEVER invent a package, price or availability. If nothing matches, say so and offer a custom runOfShow.
- Use get_package_quote for indicative pricing.
- Call capture_lead once you know the client's name and location, so nothing is lost.
...
If the caller asks whether you are a human or a bot, be honest: you're Maya, MooNs Events's virtual assistant.
Always reply in the SAME language and script style the client used. If they write Telugu or Hindi in Latin letters, reply in Latin letters too; do not unexpectedly switch to Telugu or Devanagari script. For mixed-language text, use simple English with the same familiar words they used.
Understand obvious spelling mistakes and location abbreviations (for example, UEA means UAE). Never ask the client to repeat a clear text request just because its grammar or spelling is imperfect.
When the client asks for venues or packages and gives a city or location, call find_packages immediately. If they mention wedding, corporate, conference, exhibition, concert, birthday, or celebration, pass that as the keyword. Recommend only the real event inventory returned by the tool.
...
const TURN_HINT =
  'The client just spoke (audio attached). Respond per your instructions — call tools as needed, then give your JSON reply.';
...
function toolFallback(executed: ExecutedToolCall[]): string | undefined {
  ...
  if (last.name === 'find_packages') {
    ...
    return "I couldn't find a matching ready-made package, but I can have our team build a custom runOfShow for you.";
  }
...
function instructionForChannel(ctx: MayaChannelContext, input: MayaInput) {
  const scriptInstruction =
    'text' in input && isLatinScriptMessage(input.text)
      ? '\nThe latest client message uses Latin letters. The text field in your final JSON MUST also use Latin letters only, even when the language is Telugu or Hindi.'
      : '';
...
function inferPackageLookup(input: MayaInput, history: MayaTurn[]) {
  ...
  if (!/\b(package|packages|place|places|suggest|event|events)\b/i.test(text)) return null;

  const knownLocation = text.match(/\b(hyderabad|secunderabad|warangal|vijayawada|amaravati|visakhapatnam|vizag|guntur|tirupati|rajahmundry)\b/i)?.[1];
  const trailingLocation = text.match(/\b(?:in|to|for)\s+([a-z][a-z ]{1,35})[?.!]*$/i)?.[1];
  const location = (knownLocation ?? trailingLocation)?.trim();
  ...
  const keyword = context.match(
    /\b(wedding|corporate|conference|exhibition|concert|birthday|celebration)\b/i,
  )?.[1];
  return {
    location: /^(?:uea|uae)$/i.test(location) ? 'UAE' : location,
      ...
      'audioBase64' in input
        ...
        : [
            ...
            ...(inferredPackageArgs
              ? [
                  {
                    text: `Application-provided VERIFIED inventory result: ${executed.at(-1)?.result.message}\nAnswer the client directly from this result. Do not ask for the location again and do not invent alternatives.`,
                  },
                ]
              : []),
          ];
      ...
      for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        ...
        if (calls.length === 0) {
          if (!safeResponseText(result) && emptyRetries < 1) {
            ...
            result = await chat.sendMessage([
              {
                text: 'Your previous response was empty. Complete the client request now: call the appropriate tool if needed, then return the required JSON reply.',
              },
            ]);
            continue;
          }
        ...
        for (const call of calls) {
          ...
          const toolResult = previous
            ? {
                ...previous,
                message: `${previous.message} This exact lookup already ran; use its result and answer the client now.`,
              }
