...
import { triageMessage, type Triage } from '../onevent/triage.js';
import type { MayaTurn } from '../types.js';
import { openSosRecovery } from '../../services/siteCompatibilityService.js';

let processing = false;

export function redactSensitiveEventData(text: string) {
  return text
    .replace(
      /\b(passport|visa|permit|credential|aadhaar|aadhar)\s*(?:number|no\.?|#)\s*(?:is\s*)?[:=-]?\s*[a-z0-9-]{5,20}\b/gi,
      '$1 number [REDACTED]',
    )
    .replace(
      /\b(passport|visa|permit|credential|aadhaar|aadhar)\s+(?!number\b|no\b|is\b)[a-z0-9-]{5,20}\b/gi,
      '$1 number [REDACTED]',
    )
    .replace(/\b\d{4}[ -]?\d{4}[ -]?\d{4}\b/g, '[REDACTED ID]');
}

export const redactSensitiveTravelData = redactSensitiveEventData;

function urgentAcknowledgement(text: string) {
  // Keep emergency messaging event-specific in every locale until translated event copy is supplied.
  if (/[^\u0000-\u007f]/.test(text)) {
    return "I'm Maya, MooNs Events' virtual assistant. I've alerted our on-event team immediately. Please stay somewhere safe and keep your phone with you.";
  }
  if (/[\u0900-\u097f]/.test(text)) {
    return 'मैं Maya, MooNs Events की वर्चुअल असिस्टेंट हूँ। मैंने हमारी ऑन-इवेंट टीम को तुरंत अलर्ट कर दिया है। कृपया सुरक्षित स्थान पर रहें और अपना फ़ोन पास रखें।';
  }
  if (/[\u0c00-\u0c7f]/.test(text)) {
    return 'నేను MooNs Events వర్చువల్ అసిస్టెంట్ Mayaని. మా ఆన్-ఈవెంట్ బృందాన్ని వెంటనే అప్రమత్తం చేశాను. దయచేసి సురక్షిత ప్రదేశంలో ఉండి, మీ ఫోన్ దగ్గర ఉంచుకోండి.';
  }
  return "I'm Maya, MooNs Events' virtual assistant. I've alerted our on-event team immediately. Please stay somewhere safe and keep your phone with you.";
}
  ...
  async escalate(context, triage, safeInput) {
    const recoveryReply =
      triage.category === 'booking_issue' && context.customer
        ...
        : null;
    const reply =
      typeof recoveryReply === 'string' ? recoveryReply : urgentAcknowledgement(safeInput);
    await prisma.$transaction([
      prisma.callback_requests.create({
        data: {
          name: context.customer?.name ?? context.chat.guest_name ?? 'Client',
