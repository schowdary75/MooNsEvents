...
async function canonicalConversation(request: {
  ...
  return prisma.$transaction(async (tx) => {
    const conversation = await tx.travelConversation.create({
      data: { subject: 'Website event client chat', mayaMode: 'copilot' },
    });
...
const globalChatRepository: GovernedGlobalChatRepository = {
  ...
  async escalate(context, triage) {
    const reply =
      "I'm Maya, MooNs Events' virtual assistant. I've alerted our on-event team immediately. If you are in immediate danger, contact local emergency services first and stay somewhere safe.";
