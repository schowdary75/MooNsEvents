...
/**
 * Events-insurance attach — a high-margin add-on and a genuine client
...
// Older clients cost more to cover; a simple, transparent loading factor.
...
export function estimatePremium(params: {
  ...
  const { tier, days, clients } = params;
  if (days <= 0 || clients <= 0) throw new Error('Days and clients must be positive.');
  const cfg = TIER[tier];
  const premium = Math.round(
    cfg.ratePerDay * days * clients * ageLoading(params.ageBand ?? 'under60'),
  );
...
export interface InsuranceProvider {
  ...
  quote(params: { tier: CoverageTier; days: number; clients: number }): Promise<InsuranceQuote>;
}
