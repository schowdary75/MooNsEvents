...
export interface FlightWatch {
  ...
  /** Cross-border itineraries use a higher operational-severity threshold. */
  crossBorder: boolean;
...
export interface TimelineWatch extends Partial<FlightWatch> {
  bookingId: number;
...
export function classifyDisruption(
  ...
  if (status.state === 'delayed') {
    const operationalHighThreshold = crossBorder ? 360 : 180;
...
function travellerMessage(
  ...
  const opening =
    status.state === 'cancelled'
      ? `Event timeline item ${watch.flightNumber} has been cancelled.`
      : `Heads up: event timeline item ${watch.flightNumber} is delayed by about ${Math.round(
          status.delayMinutes / 60,
        )}h ${status.delayMinutes % 60}m.`;
  const nextStep = rightsAssessment.confirmationRequired
    ? ' Our team is checking the applicable vendor, rescheduling, and contract options. We will not make a change or refund choice without your approval.'
    : ' Our team is confirming the available replacement, refund, or rescheduling choice with you now.';
...
export async function evaluateWatch(
  watch: FlightWatch | TimelineWatch,
  status: FlightStatus,
  deps: MayaDeps,
): Promise<DisruptionOutcome> {
  const { severity, autoRefundEligible } = classifyDisruption(status, watch.crossBorder);
  const passengerRights = assessPassengerRights(status, {
    ...
    crossBorder: watch.crossBorder,
  });
  const timelineNumber =
    (watch as TimelineWatch).timelineNumber ?? (watch as FlightWatch).flightNumber;
  const clientPhone =
    (watch as TimelineWatch).clientPhone ?? (watch as FlightWatch).travellerPhone;
  const normalizedWatch = {
    ...(watch as FlightWatch),
    flightNumber: timelineNumber,
    travellerPhone: clientPhone,
  };
  ...
  const existing = await deps.prisma.booking_contingencies.findFirst({
    where: { booking_id: watch.bookingId, issue_type: 'timeline_disruption', resolved_at: null },
    select: { id: true },
  });
  ...
  const contingency = await deps.prisma.booking_contingencies.create({
    data: {
      ...
      issue_type: 'timeline_disruption',
      severity,
      details:
        `Timeline ${timelineNumber} ${status.state}` +
  ...
  if (clientPhone) {
    const delivery = await deps.sendWhatsApp(
      clientPhone,
      travellerMessage(normalizedWatch, status, passengerRights),
    );
    alerted = delivery.ok;
  }
  ...
  await deps.logActivity(
    ...
    'timeline_disruption',
    contingency.id,
    `Maya opened a ${severity} contingency for booking #${watch.bookingId} (${timelineNumber} ${status.state})` +
      `${alerted ? ' and alerted the client' : ''}.`,
    'attention',
  );
