...
const recognizeCaller: MayaTool = {
  ...
  description:
    'Look up whether this phone number belongs to an existing lead so Maya can greet a returning client by name and reference their event. Call this at the start of a call when a phone number is available.',
  ...
  async execute(rawArgs, ctx) {
    ...
    return ok(
      `Returning client: ${lead.name}, previously interested in an event at ${lead.destination}` +
...
const findPackagesParams: GeminiSchema = {
  ...
  properties: {
    location: {
      type: 'string',
      description: 'Event venue, city, or service location in Telangana or Andhra Pradesh, e.g. "Hyderabad" or "Vijayawada".',
    },
    maxBudgetInr: {
      type: 'integer',
    ...
    keyword: {
      type: 'string',
      description: 'Optional event-type hint to match in the package name (e.g. "conference").',
    },
  },
  required: ['location'],
};
...
const findPackages: MayaTool = {
  name: 'find_packages',
  description:
    'Search MooNs Events real, active event-package inventory by location (and optional budget/keyword). Returns up to 5 genuine packages with live prices. Use this before quoting anything; never invent a package or price.',
  parameters: findPackagesParams,
  validate: findPackagesSchema,
  async execute(rawArgs, ctx) {
    const args = findPackagesSchema.parse(rawArgs);
    const term = args.location;
    ...
    if (matches.length === 0) {
      return ok(
        ...
        }. Offer to have a specialist build a custom run-of-show instead.`,
        { packages: [] },
      );
    }
...
const getPackageQuoteParams: GeminiSchema = {
  ...
  properties: {
    ...
    clients: { type: 'integer', description: 'Expected guest or attendee count. Defaults to 2.' },
  },
  required: ['packageId'],
};
const getPackageQuoteSchema = z.object({
  packageId: z.number().int().positive(),
  clients: z.number().int().positive().max(50000).optional(),
  travelers: z.number().int().positive().max(50000).optional(),
});

const getPackageQuote: MayaTool = {
  name: 'get_package_quote',
  description:
    'Produce an indicative price for a specific event package and attendee count using the real package price from inventory. The figure is a starting indication, not a final locked quote.',
  ...
  async execute(rawArgs, ctx) {
    ...
    const clients = args.clients ?? args.travelers ?? 2;
    const indicativeTotal = pkg.price * clients;
    return ok(
      `${pkg.name}: ${INR(pkg.price)} per person, indicative total ${INR(indicativeTotal)} for ` +
        `${clients} attendee(s) (${pkg.days} day programme). Confirm the exact quote after event dates, venue, and availability.`,
      { packageId: pkg.id, perPerson: pkg.price, clients, indicativeTotal },
    );
  },
};
...
const captureLeadParams: GeminiSchema = {
  ...
  properties: {
    ...
    location: { type: 'string', description: 'Event venue, city, or service location.' },
    eventMonth: { type: 'string', description: 'Rough event timing, e.g. "December".' },
    clients: { type: 'integer', description: 'Expected guest or attendee count.' },
...
const captureLeadSchema = z.object({
  ...
  eventMonth: z.string().trim().max(80).optional(),
  clients: z.number().int().positive().max(50000).optional(),
...
const captureLead: MayaTool = {
  ...
  async execute(rawArgs, ctx) {
    ...
    if (existing) {
      await ctx.deps.prisma.lead_submissions.update({
        ...
        data: {
          ...
          ...(args.eventMonth ? { travel_month: args.eventMonth } : {}),
          ...(args.clients ? { travelers_count: args.clients } : {}),
      ...
      const created = await ctx.deps.prisma.lead_submissions.create({
        data: {
          ...
          email: args.email ?? '',
          destination: args.location,
          travel_month: args.eventMonth ?? null,
          travelers_count: args.clients ?? 2,
    ...
    await ctx.deps.logActivity(
      ...
      leadId,
      `Maya (${ctx.channel}) ${existing ? 'updated' : 'captured'} lead "${args.name}" for ${args.location}.`,
    );
...
const sendWhatsAppParams: GeminiSchema = {
  ...
  properties: {
    ...
    summary: {
      type: 'string',
      description: 'The message body to send the client (their event details and next steps).',
    },
    packageIds: {
      type: 'array',
      items: { type: 'integer' },
...
const sendWhatsAppSummary: MayaTool = {
  ...
  description:
    'Send the client a WhatsApp message with their event summary and package details. This fulfils Maya’s promise to send the plan on WhatsApp, falling back to SMS if WhatsApp is unavailable.',
  ...
  async execute(rawArgs, ctx) {
    ...
    await ctx.deps.logActivity(
      ...
      'whatsapp_sent',
      null,
      `Maya sent an event summary to ${phone} via ${delivery.channel}.`,
...
const scheduleCallbackParams: GeminiSchema = {
  ...
  properties: {
    ...
    location: { type: 'string', description: 'Event venue or location, if relevant.' },
...
const scheduleCallback: MayaTool = {
  ...
  description:
    'Queue a callback request so an event specialist rings the client back. Use when the client asks to be called later or wants help with something Maya cannot finish now.',
  ...
  async execute(rawArgs, ctx) {
    ...
    const created = await ctx.deps.prisma.callback_requests.create({
      data: {
        name: args.name,
        phone,
        destination: args.location ?? null,
        status: 'pending',
      },
      select: { id: true },
    });
...
const escalateParams: GeminiSchema = {
  ...
  properties: {
    ...
    name: { type: 'string', description: "Client's name, if known." },
    phone: { type: 'string', description: 'Phone in E.164. Omit to use the caller.' },
  },
  required: ['reason'],
};
...
const findCatalogItems: MayaTool = {
  ...
  description:
    'Search the real event catalogue (venues, spaces, activities, and transport) by location to obtain genuine catalog ids. Feed those ids into build_custom_quote; never invent an id or a price.',
  ...
  async execute(rawArgs, ctx) {
    ...
    const where = {
      ...
      destination: { contains: args.location },
    ...
    if (rows.length === 0) {
      return ok(
        `No ${args.catalogType} options are on file for ${args.location}. Offer a custom event build; the team can source one through an RFQ.`,
        { items: [] },
      );
    }
    ...
    return ok(
      `Found ${items.length} ${args.catalogType}(s) in ${args.location}: ${items
...
const buildQuoteParams: GeminiSchema = {
  type: 'object',
  properties: {
    eventDate: { type: 'string', description: 'Event start date, ISO e.g. 2026-12-10.' },
    clients: { type: 'integer', description: 'Expected guest or attendee count.' },
    items: {
      type: 'array',
  ...
  required: ['eventDate', 'clients', 'items'],
};
const buildQuoteSchema = z.object({
  eventDate: z.string().min(1),
  clients: z.number().int().positive().max(50000),
  items: z
    .array(
      z.object({
        catalogType: z.enum(['stay', 'room', 'activity', 'car']),
...
const buildCustomQuoteTool: MayaTool = {
  ...
  description:
    'Assemble a custom event run-of-show quote (venues, production, vendors, activities, and transport) priced only from real, active rate cards. If any component lacks a live rate, the quote remains indicative and Maya must not present it as firm. The tool raises a supplier RFQ for gaps and tells the client the team will confirm the final price. Never invent prices.',
  ...
  async execute(rawArgs, ctx) {
    ...
    if (Number.isNaN(eventDate.getTime())) return fail('That event date is not valid.');
    ...
    if (quote.confidence === 'confirmed') {
      return ok(
        `Firm custom event quote ready: ${money} for ${args.clients} attendee(s); all ${quote.lines.length} components are rate-card-backed.`,
        {
