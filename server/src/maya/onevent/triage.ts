...
/**
 * 24×7 on-event support desk — Maya as first responder. During a event, a
 * client message is triaged: anything urgent (safety, a broken booking) is
...
export type EventIssueCategory = 'emergency' | 'booking_issue' | 'question' | 'feedback';
...
const ACK: Record<EventIssueCategory, string> = {
  emergency:
    "I'm getting a member of our team to call you right now — please stay where you are and keep your phone handy.",
  booking_issue:
    "I'm sorry about that. I've flagged this to our on-event team and someone will call you back very shortly to fix it.",
  feedback: "That's wonderful to hear — thank you! I'll pass it on to your event team.",
  question: "Thanks for reaching out — here to help. I'll get you an answer right away.",
};

/**
 * Handle one on-event message end to end: triage it, escalate urgent cases to a
 * human through the tool registry, and return the acknowledgement to send back.
 */
export async function handleOnEventMessage(
  deps: MayaDeps,
  ...
  if (!triage.autoReplyable) {
    const ctx = {
      ...
      sessionId: msg.sessionId ?? `onevent_${Date.now()}`,
      deps,
    };
    const result = await dispatchTool(
      'escalate_to_human',
      {
        reason: `On-event ${triage.category} (${triage.urgency}): ${msg.text}`,
