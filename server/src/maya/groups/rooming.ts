...
/**
 * Allocate `pax` clients into rooms of `roomCapacity`, filling rooms and
...
/**
 * Split a group total across clients. The remainder (from integer rupees) is
 * spread one rupee at a time across the first clients so the shares always
...
export function settlementState(
  ...
  for (const c of contributions) {
    ...
    byClient[c.clientRef] = (byClient[c.clientRef] ?? 0) + c.amountInr;
    paidInr += c.amountInr;
  }
  const remainingInr = Math.max(0, totalInr - paidInr);
  return { totalInr, paidInr, remainingInr, settled: paidInr >= totalInr, byClient };
}
