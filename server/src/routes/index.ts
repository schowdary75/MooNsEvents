...
import { aiGovernanceRoutes } from './aiGovernanceRoutes.js';
...
import { metaWhatsAppWebhookRoutes } from './metaWhatsAppWebhookRoutes.js';
...
apiRoutes.use('/public/webhooks/meta/whatsapp', metaWhatsAppWebhookRoutes);
apiRoutes.get('/public/incident-vendor-response', async (request, response, next) => {
  try {
    ...
    response
      ...
      .type('html')
      .send(
        `<!doctype html><meta charset="utf-8"><title>MooNs Events response</title><main style="font:16px system-ui;max-width:40rem;margin:4rem auto;padding:1rem"><h1>Response recorded</h1><p>Thank you. MooNs Events has recorded that you are <strong>${decision}</strong>.</p><p>You may close this page.</p></main>`,
      );
    void result;
  } catch (error) {
    next(error);
...
apiRoutes.use('/events-governance', authenticate, tenantScope, aiGovernanceRoutes);
...
apiRoutes.use(
  ...
  authorize('admin', 'editor', 'manager', 'sales', 'support'),
  whatsappRoutes,
);
