...
import { EVENT_UPDATES_CHANNEL, type EventRealtimeMessageMessage } from '../services/eventRealtimeService.js';

let io: Server | null = null;
const OPERATIONS_PRESENCE_CHANNEL = 'moonsevents:operations-presence';
...
function presenceKey(tenantId?: string) {
  return `moonsevents:operations-presence:${tenantId ?? 'legacy'}`;
}
...
async function readPresence(tenantId?: string) {
  ...
  const connectedAgents = new Set(members.map((member: string) => member.split(':', 1)[0])).size;
...
export function initializeSocket(server: HttpServer) {
    ...
    if (auth.principalType === 'crm_user') {
      ...
      socket.on('disconnect', () => {
        ...
        void redis
          ...
          .then(() => broadcastPresence(auth.tenantId))
          .catch((error: unknown) => logger.warn('Unable to clear operations presence', { error }));
      });
    } else {
  ...
  try {
    ...
    void subscriber
      ...
      .catch((error: unknown) => logger.warn('Redis pubsub subscription skipped', { error }));
  } catch (error) {
    logger.warn('Redis duplicate skipped', { error });
  }
  return io;
}
