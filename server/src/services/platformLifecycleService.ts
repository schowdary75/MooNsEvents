...
async function destroyExpiredTenant(tenant: {
  ...
  if (
    !databaseName.startsWith('moonsevents_') ||
      ...
      for (const trial of trials) {
        if (
          await notice(
            trial.tenantId,
            action,
            `Your MooNsEvents trial ends in ${hours} hours`,
    ...
    for (const trial of expiredTrials) {
      ...
      if (!paid) {
        ...
        await notice(
          trial.tenantId,
          'trial.expired',
          'Your MooNsEvents trial has ended',
      ...
      if ([30, 7, 1].includes(days)) {
        if (
          await notice(
            ...
            `MooNsEvents data deletion in ${days} day${days === 1 ? '' : 's'}`,
