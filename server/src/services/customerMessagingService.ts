...
/** Normalize a stored (country-code-seventped) number to E.164 for the gateway. */
 ...
 * and throttled to avoid eventping carrier spam limits.
 *
...
export async function handleInboundSms(from: string, text: string): Promise<{ action: string }> {
  ...
  if (STOP_WORDS.has(word)) {
    ...
    await smsService.sendSMS(
      ...
      'You have been unsubscribed from MooN Events promotions. Reply START to opt back in.',
    );
  ...
  if (START_WORDS.has(word)) {
    ...
    await smsService.sendSMS(
      normalizeForSms(from),
      'You are re-subscribed to MooN Events updates. Reply STOP to opt out anytime.',
    );
