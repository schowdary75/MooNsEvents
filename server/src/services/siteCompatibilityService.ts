...
export function classifySosIssue(content: string) {
  const venue =
    /Venue Issue/i.test(content) ||
    /\b(venue|hall|room|space|check[ -]?in)\b.*\b(overbook|no (?:booking|room|space)|refus|closed|unavailable)\b/i.test(
      content,
    );
  const transport =
  ...
  if (venue) return 'venue_issue' as const;
...
export async function openSosRecovery(customerId: number, content: string) {
  ...
  for (const booking of bookings) {
    try {
      ...
      return [
        ...
        `Current status: ${status.replace(/_/g, ' ')}. I am contacting the assigned ${issueType === 'venue_issue' ? 'venue' : 'transport provider'} and tracking a 10-minute response deadline.`,
  ...
  return [
    'I received the SOS but could not safely start automated recovery because this account has no active confirmed event booking attached to it.',
    'The support team has been alerted through this chat. Please share your booking reference and venue location. No replacement or reimbursement has been authorized yet.',
  ].join('\n\n');
}
...
export const siteCompatibilityService = {
  ...
  async sendTeamMessage(auth: AuthInput, content: string) {
    ...
    await repository.createTeamMessage(staff.id, content);
    return { success: true };
  },
