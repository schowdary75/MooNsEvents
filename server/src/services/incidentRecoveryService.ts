    ...
    try {
      await emailQueue.add(
        'incident-customer-update',
        {
          tenantId,
          to: customer.email,
          subject: `MooNs Events SOS update: ${issueName(
...
async function contactVendor(
  ...
  const message = [
    `URGENT MooNs Events ${issueName(recovery.issueType)} for booking ${booking.booking_reference}.`,
...
export async function vendorCallPrompt(attemptId: string) {
  ...
  const prompt = `Urgent service recovery call from MooNs Events for booking ${booking.booking_reference}. We have a ${issueName(
    recovery.issueType,
  ...
  await prisma.$transaction([
    prisma.incidentRecovery.update({
      ...
      data: {
        ...
        resolvedAt: new Date(),
        responseDueAt: null,
        resolutionSummary: 'Client confirmed that the replacement or assigned event service was received.',
      },
    }),
