...
/** Build an E.164 recipient for the SMS gateway from a stored (country-code-seventped) number. */
function phoneForSms(storedPhone: string): string {
  ...
  // Stored numbers have +91 seventped; a bare 10-digit number is Indian.
...
export const customerAuthService = {
  ...
  async requestOtp(phoneInput: string, emailInput: string) {
    ...
    const subject = 'Confirm your MooNs Events account';
    const smsBody = `Your MooNs verification code is ${code}. It expires in 5 minutes.`;
    const body = [
      'Confirm your MooNs Events account',
      '',
      `Your one-time verification code is: ${code}`,
      '',
      'This code expires in 5 minutes and can be used only once.',
      'If you did not try to create a MooNs Events account, you can safely ignore this email.',
      '',
      'MooNs Events',
      'events.moon.com',
    ].join('\n');
...
<html lang="en">
  <body style="margin:0;background:#f5f3ef;color:#20150f;font-family:Arial,sans-serif">
    <div style="display:none;max-height:0;overflow:hidden">Use ${code} to confirm your MooNs Events account. The code expires in 5 minutes.</div>
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:520px;background:#ffffff;border:1px solid #e2ddd7;border-radius:16px">
            <tr>
              <td style="padding:32px">
                <p style="margin:0 0 24px;font-size:20px;font-weight:700">MooNs Events</p>
                ...
                <p style="margin:0;color:#655c56;font-size:13px;line-height:1.6">If you did not try to create a MooNs Events account, you can safely ignore this email.</p>
              </td>
            </tr>
          </table>
        </td>
