...
export const careerOperationService = {
  ...
  async updateStatus(input: {
    ...
    if (input.status === 'shortlisted') {
      ...
      await sendEmail({
        to: input.email,
        subject: `Interview Invitation: ${input.jobTitle} at MooN Events`,
