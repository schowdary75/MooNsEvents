  ...
  async createInvoice(input: {
    ...
    const contactResult = await request('/contacts', {
      ...
      body: JSON.stringify({
        ...
        custom_fields: [{ label: 'MooNsEvents Tenant ID', value: input.tenantId }],
      }),
      headers: {
        'X-Unique-Identifier-Key': 'custom_fields.MooNsEvents Tenant ID',
    ...
    const invoiceResult = await request('/invoices', {
      ...
      body: JSON.stringify({
        ...
        notes: `MooNsEvents subscription for tenant ${input.tenantId}`,
      }),
    });
