...
export const tenantBackupService = {
  async create(tenantId: string) {
    ...
    const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'moonsevents-backup-'));
  ...
  async restoreDrill(artifactId?: string) {
    ...
    const temporary = await fs.mkdtemp(path.join(os.tmpdir(), 'moonsevents-restore-'));
    const file = path.join(temporary, `${artifact.id}.sql.gz`);
    const database = `moonsevents_restore_${randomUUID().replace(/-/g, '').slice(0, 12)}`;
