...
// full detail; the text is a short heads-up that points back to My Events).
function bookingSmsText(kind: BookingEmailKind, v: Record<string, string>): string {
  const name = v.customer_name || 'client';
  const ref = v.reference || '';
  switch (kind) {
    case 'booking-received':
      return `Hi ${name}, we've received your MooN Events booking ${ref} for ${v.item_name} (${v.amount}). We'll confirm shortly — track it on My Events.`;
    case 'booking-confirmed':
      return `Hi ${name}, your MooN Events booking ${ref} for ${v.item_name} is CONFIRMED (events date ${v.event_date}). Details on My Events.`;
    case 'booking-payment-issue':
      return `Hi ${name}, there's a payment issue with your MooN booking ${ref}. Please check your email or My Events to resolve it.`;
    case 'booking-invoice':
      return `Hi ${name}, tax invoice ${v.invoice_number} (${v.amount}) for booking ${ref} has been emailed to you. Thank you — MooN Events.`;
    default:
      return `Update on your MooN Events booking ${ref}. See your email or My Events for details.`;
  }
}
...
const BRAND_HEADER = `
  <div style="background:#111014;padding:28px 32px;border-radius:16px 16px 0 0;">
    ...
    <span style="display:block;margin-top:4px;font-size:11px;letter-spacing:3px;text-transform:uppercase;color:#8a8578;">Events · Escrow-shielded bookings</span>
  </div>`;
...
const BRAND_FOOTER = `
  <div style="padding:20px 32px;border-top:1px solid #eee7d9;color:#8a8578;font-size:11px;line-height:1.7;">
    ...
    Track everything anytime on your <strong>My Events</strong> page. Reply to this email or ping your WhatsApp Events Buddy for help.<br/>
    © MooN Events — Explore. Experience. Repeat.
  </div>`;
...
function detailsTable(vars: BookingEmailVars) {
  const rows: Array<[string, string | undefined]> = [
    ...
    ['Event', vars.item_name],
    ['Amount', vars.amount],
    ['Events date', vars.event_date],
...
export function invoiceDocument(vars: BookingEmailVars) {
  ...
  <div style="border:1px solid #e8e0cf;border-radius:12px;overflow:hidden;margin:6px 0 4px;">
    ...
    <div style="padding:16px 22px;border-bottom:1px solid #e8e0cf;background:#ffffff;">
      <strong style="color:#111014;font-size:14px;display:block;margin-bottom:4px;">MooN Events & Expeditions Pvt. Ltd.</strong>
      <span style="color:#8a8578;font-size:12px;display:block;">123 Events Boulevard, Wanderlust City<br/>State: WL (10203), India<br/>GSTIN: 22AAAAA0000A1Z5 | hello@moonexpeditions.com</span>
    </div>
    ...
    <table style="width:100%;border-collapse:collapse;">
      <tr style="background:#111014;">
        ...
        <td style="padding:10px 12px;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:#f5f0e8;text-align:center;">Events Date</td>
      ...
      <tr>
        <td style="padding:14px 22px;font-size:13px;color:#111014;font-weight:600;border-bottom:1px solid #f2ecdf;">${vars.item_name}<span style="display:block;font-size:11px;color:#8a8578;font-weight:normal;margin-top:2px;">Curated events package · escrow-shielded</span></td>
        <td style="padding:14px 12px;font-size:13px;color:#33302a;text-align:center;border-bottom:1px solid #f2ecdf;">9985</td>
        <td style="padding:14px 12px;font-size:13px;color:#33302a;text-align:center;border-bottom:1px solid #f2ecdf;">${vars.event_date}</td>
      ...
      <tr>
        <td style="padding:14px 22px;vertical-align:top;">
          ...
          <div style="margin-top:16px;">
            ...
            <span style="display:block;font-size:11px;color:#33302a;line-height:1.5;">Bank: HDFC Bank, Wanderlust Branch<br/>A/C Name: MooN Events Escrow Account<br/>A/C No: 50200000000000<br/>IFSC: HDFC0000001</span>
          </div>
        </td>
...
const DEFAULTS: Record<BookingEmailKind, { subject: string; body: string }> = {
  'booking-received': {
    ...
    body:
      ...
      `{{details_table}}` +
      `<p>Once verified, you'll get a confirmation email and your WhatsApp Events Buddy will reach out with next steps.</p>`,
  },
  'booking-confirmed': {
    ...
    body:
      ...
      `{{details_table}}` +
      `<p>Your amount is held in MooN's escrow shield and released to operators only as your event milestones complete. Your Events Buddy will WhatsApp you the final documents and day-wise plan.</p>`,
  },
  'booking-invoice': {
    ...
    body:
      ...
      `<p style="margin:0 0 16px;">Thank you for booking with MooN Events. Your payment has been verified — here is your tax invoice for booking <strong>{{reference}}</strong>. Keep it safe for your records.</p>` +
      `{{invoice_document}}` +
      `<p style="margin:16px 0 0;">Questions about this invoice? Just reply to this email or message your WhatsApp Events Buddy — Maya and the team respond within a few hours.</p>`,
  },
...
export async function sendBookingEmail(
  ...
  try {
    ...
    const flat: Record<string, string> = {
      customer_name: vars.customer_name || 'client',
    ...
    const info = await transporter.sendMail({
      from: '"MooN Events" <' + (process.env.SMTP_FROM || 'hello@moonsevents.com') + '>',
      to,
...
async function sendBookingSms(kind: BookingEmailKind, vars: BookingEmailVars): Promise<void> {
  ...
  const flat: Record<string, string> = {
    customer_name: vars.customer_name || 'client',
    reference: vars.reference || '',
    item_name: vars.item_name || 'your event',
    amount: vars.amount || '',
    event_date: vars.event_date || '',
    invoice_number: vars.invoice_number || '',
  };
...
// Match customerAuthService.phoneForSms: stored numbers have +91 seventped.
