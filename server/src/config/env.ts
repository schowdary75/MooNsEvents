...
if (!process.env.DATABASE_URL && process.env.DATABASE_HOST) {
  ...
  const database = process.env.DATABASE_NAME || 'moonsevents';
...
if (process.env.DATABASE_URL) {
  if (!process.env.PLATFORM_DATABASE_URL) {
    ...
    platformUrl.pathname = '/moonsevents_platform';
...
if (!process.env.TENANT_CREDENTIAL_ENCRYPTION_KEY && process.env.JWT_ACCESS_SECRET) {
  process.env.TENANT_CREDENTIAL_ENCRYPTION_KEY = createHash('sha256')
    .update(`moonsevents:tenant-credentials:${process.env.JWT_ACCESS_SECRET}`)
    .digest('hex');
}
...
const schema = Joi.object({
  ...
  DATABASE_URL: Joi.string()
    .uri()
    .default('mysql://moon_user:change-me@127.0.0.1:3306/moonsevents'),
  PLATFORM_DATABASE_URL: Joi.string()
    .uri()
    .default('mysql://moon_user:change-me@127.0.0.1:3306/moonsevents_platform'),
  ...
  JWT_ISSUER: Joi.string().default('moonsevents'),
  JWT_AUDIENCE: Joi.string().default('moonsevents-client'),
  ...
  MFA_ENROLLMENT_REQUIRED: Joi.boolean().truthy('true').falsy('false').default(true),
  ...
  EVENT_AUTOMATION_CRON: Joi.string().default('*/30 * * * * *'),
  ...
  META_WHATSAPP_API_VERSION: Joi.string()
    .pattern(/^v\d+\.\d+$/)
    .default('v21.0'),
  EVENT_POLICY_PROVIDER: Joi.string()
    .valid('timatic', 'official_sources', 'disabled')
    .default('disabled'),
  EVENT_POLICY_API_KEY: Joi.string().allow('').default(''),
...
export const env = {
  ...
  mfaEnrollmentRequired: value.MFA_ENROLLMENT_REQUIRED as boolean,
  ...
  rateLimit: {
    ...
    auth: value.AUTH_RATE_LIMIT as number,
  ...
  cron: {
    ...
    eventAutomation: value.EVENT_AUTOMATION_CRON as string,
  ...
  metaWhatsapp: {
    accessToken: value.META_WHATSAPP_TOKEN as string,
    phoneNumberId: value.META_WHATSAPP_PHONE_NUMBER_ID as string,
    businessAccountId: value.META_WHATSAPP_BUSINESS_ACCOUNT_ID as string,
    appSecret: value.META_WHATSAPP_APP_SECRET as string,
    verifyToken: value.META_WHATSAPP_VERIFY_TOKEN as string,
    apiVersion: value.META_WHATSAPP_API_VERSION as string,
  },
  eventProviders: {
    timelineStatus: {
    ...
    whatsapp: {
      configured: Boolean(
        ...
        value.META_WHATSAPP_BUSINESS_ACCOUNT_ID &&
        value.META_WHATSAPP_APP_SECRET,
      ),
    },
    eventRules: {
      provider: value.EVENT_POLICY_PROVIDER as 'timatic' | 'official_sources' | 'disabled',
      configured:
        value.EVENT_POLICY_PROVIDER === 'official_sources' || Boolean(value.EVENT_POLICY_API_KEY),
    },
    insurance: {
