...
let recoveryVoiceWorker: Worker | null = null;
...
try {
  ...
  recoveryVoiceWorker = createRecoveryVoiceWorker(ariService);
  recoveryVoiceWorker.on('failed', (job, error) =>
    logger.error('Recovery voice job failed', { jobId: job?.id, error }),
  );
} catch (err) {
  logger.warn('Optional background initialization error', { error: err });
}
...
async function shutdown(signal: string) {
  ...
  server.close(async () => {
    const shutdownTasks: Array<Promise<unknown>> = [
      ...
      redis.quit(),
      recoveryVoiceWorker?.close() ?? Promise.resolve(),
    ];
    if (env.cron.enabled) {
