...
export async function reconcileRecordings(): Promise<number> {
  ...
  try {
    ...
    for (const fileName of entries) {
      ...
      const existing = await (prisma as any).call_recordings?.findUnique({
        where: { uniqueid: meta.uniqueid },
      });
      if (existing) continue;

      try {
        await (prisma as any).call_recordings?.create({
          data: {
...
export async function listRecordings(limit = 100, offset = 0) {
  return (prisma as any).call_recordings?.findMany({
...
export async function getRecording(id: number) {
  return (prisma as any).call_recordings?.findUnique({ where: { id } }) ?? null;
}
