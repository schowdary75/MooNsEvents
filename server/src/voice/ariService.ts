...
const GREETING_TEXT =
  'Hi, thanks for calling MooNs Events! This is Maya. How can I help you today?';
...
async function synthesizeGoogleTranslateSpeech(
  ...
  const response = await fetch(url, {
    headers: { 'user-agent': 'MooNsEvents/1.0' },
...
export class AriService {
  ...
  private appName = 'moonsevents_voice';
  ...
  private async processTurn(channel: ChannelInstance) {
    ...
    liveRecording.once('RecordingFinished', async () => {
      ...
      try {
        ...
        // Cover the model round-event (which may now include tool calls) with a
  ...
  private async persistCallSummary(channelId: string): Promise<void> {
    try {
      ...
      const lead = await (this.deps.prisma as any).lead_submissions?.findFirst({
      ...
      if (lead) {
        await (this.deps.prisma as any).lead_submissions?.update({
          where: { id: lead.id },
          data: {
   ...
   */
  async dialOutbound(number: string, callerId: string = 'MooNs Events') {
    if (!this.ariClient) {
