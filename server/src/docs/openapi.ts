...
const schemas = {
  ...
  WhatsAppMessage: {
    type: 'object',
    required: ['id', 'direction', 'kind', 'body', 'deliveryStatus', 'createdAt'],
    properties: {
      id: { type: 'string', format: 'uuid' },
      direction: { type: 'string', enum: ['inbound', 'outbound', 'internal'] },
      senderType: { type: 'string' },
      senderRef: { type: ['string', 'null'] },
      senderName: { type: ['string', 'null'] },
      kind: { type: 'string', enum: ['text', 'template', 'unsupported'] },
      body: { type: 'string' },
      deliveryStatus: {
        type: 'string',
  ...
  WhatsAppMessageInput: {
    oneOf: [
      {
        type: 'object',
        required: ['type', 'text', 'clientMessageId'],
        properties: {
      ...
      {
        type: 'object',
        required: ['type', 'template', 'clientMessageId'],
        properties: {
          type: { type: 'string', const: 'template' },
          clientMessageId: { type: 'string', format: 'uuid' },
          template: {
            type: 'object',
            required: ['name', 'language', 'parameters'],
            properties: {
              name: { type: 'string' },
              language: { type: 'string' },
              parameters: { type: 'array', items: { type: 'string' }, maxItems: 20 },
            },
          },
        },
      },
...
export const openapi = {
  openapi: '3.1.0',
  info: {
    title: 'MooNsEvents Enterprise API',
    version: '1.0.0',
    description: 'Versioned API running alongside the legacy MooNsEvents runtime.',
  },
  ...
  paths: {
    ...
    '/whatsapp/leads/{leadId}/conversation': {
      get: {
        ...secured,
        tags: ['WhatsApp'],
        summary: 'Get the paginated WhatsApp conversation for a lead',
        parameters: [
        ...
        responses: {
          '200': response('Conversation, provider eligibility, unread count, and messages'),
          '401': { $ref: '#/components/responses/Unauthorized' },
          '404': response('Lead not found'),
        },
      },
    },
    '/whatsapp/leads/{leadId}/messages': {
      post: {
        ...secured,
        tags: ['WhatsApp'],
        summary: 'Send a free-form or approved-template WhatsApp message',
        parameters: [{ name: 'leadId', in: 'path', required: true, schema: { type: 'integer' } }],
        requestBody: {
          required: true,
          content: json({ $ref: '#/components/schemas/WhatsAppMessageInput' }),
        },
        responses: {
          '201': response('Message accepted', { $ref: '#/components/schemas/WhatsAppMessage' }),
          '409': response('Provider unavailable or approved template required'),
          '422': response('Invalid recipient or template'),
        },
      },
    },
    '/whatsapp/leads/{leadId}/read': {
      post: {
        ...secured,
        tags: ['WhatsApp'],
        summary: 'Mark inbound WhatsApp messages read',
        parameters: [{ name: 'leadId', in: 'path', required: true, schema: { type: 'integer' } }],
        responses: { '200': response('Unread messages marked read') },
      },
    },
    '/whatsapp/templates': {
      get: {
        ...secured,
        tags: ['WhatsApp'],
        summary: 'List approved Meta WhatsApp templates',
        responses: { '200': response('Approved templates') },
      },
    },
    '/public/webhooks/meta/whatsapp': {
      get: {
        tags: ['WhatsApp'],
        summary: 'Complete Meta webhook challenge verification',
        parameters: [
        ...
        responses: { '200': response('Challenge response'), '403': response('Invalid token') },
      },
      post: {
        tags: ['WhatsApp'],
        summary: 'Receive signed Meta WhatsApp messages and delivery updates',
        parameters: [
          {
        ...
        responses: {
          '200': response('Event accepted'),
          '401': response('Invalid signature'),
          '404': response('Phone-number id is not registered'),
        },
      },
    },
  },
