...
const wishlistItem = Joi.object({
  ...
  itemType: Joi.string().valid('package', 'stay', 'car').required(),
...
export const createEventIncidentSchema = Joi.object({
  body: Joi.object({
    issueType: Joi.string().valid('transport_no_show', 'venue_issue').required(),
...
export const incidentReceiptUploadSchema = Joi.object({
  body: Joi.object({
    ...
    checksumSha256: Joi.string()
      ...
      .optional(),
    expenseType: Joi.string().valid('transport', 'venue').required(),
...
export const eventDocumentUploadSchema = Joi.object({
  body: Joi.object({
    eventId: Joi.string()
      .guid({ version: ['uuidv4'] })
      .optional(),
    ...
    documentType: Joi.string()
      .valid('credential', 'permit', 'id', 'insurance', 'ticket', 'voucher', 'medical', 'other')
      .required(),
    filename: Joi.string().trim().min(1).max(255).required(),
...
export const eventDocumentIdSchema = Joi.object({
  body: Joi.object(),
  params: Joi.object({
