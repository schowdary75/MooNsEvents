...
describe('platform business policy', () => {
  ...
  it('requires exact governed confirmation after harmless case and whitespace normalization', () => {
    expect(confirmationMatches('DELETE acme-showcases', ' delete ACME-SHOWCASES ')).toBe(true);
    expect(confirmationMatches('acme-showcases', 'another-workspace')).toBe(false);
  });
