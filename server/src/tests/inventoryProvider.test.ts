...
describe('configurable inventory provider', () => {
  it('maps a validated success response and sends the documented request contract', async () => {
    ...
    await expect(provider.searchVenues(query)).resolves.toEqual({
      available: true,
      provider: 'stub-provider',
      offers: [
        {
    ...
    expect(url).toBe('https://inventory.example.invalid/venues/search');
  ...
  it('aborts at the configured timeout and reports only a safe reason', async () => {
    ...
    const pending = provider.searchVenues(query);
  ...
  it('falls back on a network failure without logging the error object or request data', async () => {
    ...
    await expect(provider.searchVenues(query)).resolves.toMatchObject({
      available: false,
      provider: 'stub-provider',
      offers: [],
    });
  ...
  it('rejects malformed JSON and structurally invalid offers', async () => {
    ...
    await expect(malformedJson.provider.searchVenues(query)).resolves.toMatchObject({
      available: false,
      offers: [],
    });
    ...
    const malformedPayload = harness(async () =>
      response(200, { offers: [{ venueName: 'missing required fields' }] }),
    );
    await expect(malformedPayload.provider.searchVenues(query)).resolves.toMatchObject({
      available: false,
      offers: [],
    });
  ...
  it('uses the catalogue fallback when configuration is missing or reserved for examples', async () => {
    ...
    await expect(missing.searchVenues(query)).resolves.toEqual({
      available: false,
      provider: 'fallback',
      offers: [],
    });
    await expect(reserved.searchVenues(query)).resolves.toEqual({
      available: false,
      provider: 'fallback',
      offers: [],
    });
...
describe('credential-free example adapter', () => {
  it('maps injected fixtures and watermarks them as not bookable', async () => {
    ...
    const result = await provider.searchVenues(query);
