...
function createSupportHarness(message = 'What time is my pickup tomorrow?', mayaMode = 'copilot') {
  const context: SupportChatProcessorContext = {
    ...
    customer: { id: 42, name: 'Sample Client', phone: '+10000000000' },
  ...
  const repository: GovernedSupportChatRepository = {
    ...
    escalate: vi.fn(async (_context, triage, safeInput) => {
      ...
      return "I've alerted our on-event team immediately. Please stay somewhere safe.";
    }),
...
describe('governed legacy support chat processor', () => {
  ...
  it('redacts sensitive identifiers before persistence and provider prompts', async () => {
    const harness = createSupportHarness('Please check credential number A1234567.');
    ...
    expect(harness.inboundBodies).toEqual(['Please check credential number [REDACTED].']);
    expect(harness.replyProvider.respond).toHaveBeenCalledWith(
      expect.objectContaining({
        safeInput: 'Please check credential number [REDACTED].',
  ...
  it('acknowledges an urgent client and creates a staff-visible escalation', async () => {
    const harness = createSupportHarness('I lost credential number A1234567 and I am stranded.');

    await expect(harness.process(7)).resolves.toBe(true);

    expect(harness.replyProvider.respond).not.toHaveBeenCalled();
    expect(harness.replies[0]).toContain('alerted our on-event team');
    expect(harness.context.conversation.mayaMode).toBe('human_only');
    expect(harness.staffEvents).toEqual([
      {
        action: 'urgent_support_escalation',
        category: 'emergency',
        input: 'I lost credential number [REDACTED] and I am stranded.',
      },
    ]);
  });
...
describe('governed website chat processor', () => {
  ...
  it('redacts sensitive identifiers before persistence and provider prompts', async () => {
    const harness = createGlobalHarness('Please check permit number B7654321.');
    harness.context.messages.unshift({
      id: 200,
      sender_id: 'sample-lead',
      sender_type: 'lead',
    ...
    expect(harness.inboundBodies).toEqual(['Please check permit number [REDACTED].']);
    expect(harness.replyProvider.respond).toHaveBeenCalledWith(
      expect.objectContaining({
        safeInput: 'Please check permit number [REDACTED].',
  ...
  it('acknowledges an urgent client and creates a staff-visible escalation', async () => {
    const harness = createGlobalHarness('I am stranded after losing my credential.');
