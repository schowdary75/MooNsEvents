...
import { assessCredential, InMemoryDocumentVault } from '../maya/documents/credential.js';
...
describe('unified client profile', () => {
  it('stitches leads, CRM client and customer account into one view', async () => {
    ...
    expect(profile.preferences.locationsOfInterest.sort()).toEqual(['Bali', 'Goa']);
...
describe('credential six-month rule', () => {
  const now = new Date('2026-07-22T00:00:00Z');
  const events = new Date('2026-12-10T00:00:00Z');

  it('passes when 6+ months valid beyond events', () => {
    const a = assessCredential(new Date('2027-08-01T00:00:00Z'), events, now);
    expect(a.valid).toBe(true);
    expect(a.alert).toBeNull();
  });

  it('flags when under six months validity at events', () => {
    const a = assessCredential(new Date('2027-02-01T00:00:00Z'), events, now);
  ...
  it('flags an already-expired credential', () => {
    const a = assessCredential(new Date('2026-01-01T00:00:00Z'), events, now);
...
describe('document vault (fallback)', () => {
  it('stores and lists documents per client', async () => {
    const vault = new InMemoryDocumentVault();
    await vault.put({ clientRef: 't1', type: 'credential', fileUrl: '/x.pdf', expiresOn: null });
...
describe('insurance estimator', () => {
  it('scales premium by tier, days, clients and age', () => {
    const base = estimatePremium({ tier: 'standard', days: 10, clients: 2 });
    expect(base.premiumInr).toBe(75 * 10 * 2);
    const senior = estimatePremium({ tier: 'standard', days: 10, clients: 2, ageBand: 'over70' });
    expect(senior.premiumInr).toBe(base.premiumInr * 2);
  });

  it('returns all three tiers ordered', () => {
    const tiers = estimateAllTiers({ days: 5, clients: 1 });
