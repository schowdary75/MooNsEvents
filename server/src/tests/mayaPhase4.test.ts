...
import {
  ...
  isTerminal,
  type PermitCase,
} from '../maya/permits/permitCase.js';
...
import { handleOnEventMessage, triageMessage } from '../maya/onevent/triage.js';
import { needsTranslation } from '../maya/i18n/localize.js';

describe('permit case state machine', () => {
  ...
  it('advances a case and rejects illegal jumps', () => {
    ...
    const good = advancePermitCase(c, 'under_review', new Date('2026-07-22'));
    expect(good.ok).toBe(true);
    expect(c.status).toBe('under_review');

    const bad = advancePermitCase(c, 'documents_pending');
    expect(bad.ok).toBe(false);
    expect(bad.error).toContain('Cannot move');
  });

  it('flags at-risk cases close to events', () => {
    const near: PermitCase = {
      status: 'under_review',
      location: 'UK',
      eventDate: new Date('2026-07-30'),
      updatedAt: new Date(),
    };
    const far: PermitCase = {
      status: 'under_review',
      location: 'UK',
...
describe('group rooming & payments', () => {
  it('allocates clients into rooms leaving at most one partial', () => {
  ...
  it('tracks partial payments to settlement', () => {
    const state = settlementState(100000, [
      { clientRef: 'a', amountInr: 40000 },
      { clientRef: 'b', amountInr: 30000 },
    ]);
...
describe('on-event triage', () => {
  it('classifies emergencies as critical and non-auto-replyable', () => {
    expect(triageMessage('I lost credential and I am stranded!')).toMatchObject({
      category: 'emergency',
      urgency: 'critical',
      autoReplyable: false,
    });
  ...
  it('escalates urgent on-event messages through the tool layer', async () => {
    ...
    const res = await handleOnEventMessage(deps, {
      phone: '+9199',
      name: 'Sam',
      text: 'My venue has no booking for me and I am stranded',
    });
