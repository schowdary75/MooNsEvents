...
import type { TimelineStatus } from '../maya/disruption/timelineStatusProvider.js';
import {
  computeRefundSla,
  reviewRefunds,
  type RefundRow,
...
const status = (over: Partial<TimelineStatus>): TimelineStatus => ({
...
describe('disruption shield — evaluateWatch', () => {
  ...
  it('opens a contingency case and alerts the client on cancellation', async () => {
  ...
  it('does not double-open when an unresolved case already exists', async () => {
    const { deps, sendWhatsApp } = makeDeps({
      contingencies: [
        { id: 1, booking_id: 42, issue_type: 'timeline_disruption', resolved_at: null },
      ],
    });
    const outcome = await evaluateWatch(watch, status({ state: 'cancelled' }), deps);
  ...
  it('stays silent for on-time timelines', async () => {
