...
describe('rate-card validity', () => {
  it('accepts an active, in-validity, min-pax-satisfied card', () => {
    expect(assessRateCard(rc({}), 2, EVENTS).usable).toBe(true);
  });
  it('rejects expired, inactive or below-min-pax cards', () => {
    expect(assessRateCard(rc({ validTo: new Date('2026-01-01') }), 2, EVENTS).reasons).toContain(
      'rate expired',
    );
    expect(assessRateCard(rc({ isActive: false }), 2, EVENTS).usable).toBe(false);
    expect(assessRateCard(rc({ minPax: 4 }), 2, EVENTS).reasons[0]).toContain('minimum');
  });
});
...
describe('pricing maths', () => {
  ...
  it('prices a bindable line and flags a gap when no card', () => {
    const line = priceLine({ catalogType: 'stay', catalogId: 1, quantity: 3 }, rc({}), 2, EVENTS);
    expect(line.bindable).toBe(true);
    expect(line.totalSelling).toBe(15000); // 5000 * 3 nights
    const gap = priceLine({ catalogType: 'stay', catalogId: 9, quantity: 3 }, null, 2, EVENTS);
...
describe('quote confidence rollup', () => {
  ...
  const gapLine = priceLine({ catalogType: 'car', catalogId: 2, quantity: 1 }, null, 2, EVENTS);
...
describe('buildCustomQuote (service)', () => {
  ...
  it('stays indicative and lists gaps when a rate is missing', async () => {
    ...
    const quote = await buildCustomQuote(deps, {
      ...
      eventDate: EVENTS,
      items: [
        { catalogType: 'stay', catalogId: 1, quantity: 2, label: 'Venue' },
        { catalogType: 'car', catalogId: 2, quantity: 1, label: 'EventSite transfer' },
      ],
    });
    expect(quote.confidence).toBe('indicative');
    expect(quote.gaps.map((g) => g.label)).toEqual(['EventSite transfer']);
