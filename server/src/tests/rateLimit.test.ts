...
describe('operation rate limiting', () => {
  ...
  it('does not let one busy operation exhaust unrelated operation buckets', () => {
    const request = {
      ip: '127.0.0.1',
      path: '/adminGetVendorsAll',
      params: { operationName: 'adminGetVendorsAll' },
      body: { data: { auth: { sessionToken: 'admin-session' } } },
    } as unknown as Request;

    expect(operationRateLimitKey(request)).not.toBe(
      operationRateLimitKey({
        ...request,
        path: '/adminGetVendorCommunications',
        params: { operationName: 'adminGetVendorCommunications' },
      } as Request),
    );
  });
