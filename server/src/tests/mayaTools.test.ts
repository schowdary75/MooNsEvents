      ...
      findMany: vi.fn(async ({ where, take }: any) => {
        ...
        if (where.OR) {
          rows = rows.filter((p) =>
            where.OR.some(
              (condition: any) =>
                contains(p.location, condition.destination ?? condition.location) &&
...
describe('Maya tools — find_packages', () => {
  it('returns only real, active packages matching the location', async () => {
    const { ctx } = makeCtx();
    const res = await dispatchTool('find_packages', { location: 'Goa' }, ctx);
  ...
  it('respects the budget ceiling', async () => {
    ...
    const res = await dispatchTool(
      'find_packages',
      { location: 'Goa', maxBudgetInr: 30000 },
      ctx,
    );
  ...
  it('never invents a package when nothing matches', async () => {
    const { ctx } = makeCtx();
    const res = await dispatchTool('find_packages', { location: 'Antarctica' }, ctx);
    expect(res.ok).toBe(true);
    expect(res.data?.packages).toEqual([]);
    expect(res.message.toLowerCase()).toContain('custom run-of-show');
  });

  it.each(['UAE', 'UEA'])('resolves %s to United Arab Emirates inventory', async (location) => {
    const { ctx } = makeCtx();
    const res = await dispatchTool('find_packages', { location, keyword: 'family' }, ctx);
    expect(res.ok).toBe(true);
...
describe('Maya tools — capture_lead', () => {
  it('creates a new lead then de-duplicates on the same phone', async () => {
    ...
    const first = await dispatchTool(
      'capture_lead',
      { name: 'Asha', location: 'Goa', clients: 2 },
      ctx,
    );
    ...
    const second = await dispatchTool(
      'capture_lead',
      { name: 'Asha R', location: 'Goa', budgetRange: '₹1L' },
      ctx,
    );
