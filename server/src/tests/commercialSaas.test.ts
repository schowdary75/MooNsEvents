...
describe('commercial SaaS invariants', () => {
  it('creates stable, safe and tenant-unique database identifiers', () => {
    ...
    expect(first.databaseName).toBe('moonsevents_acme_showcases_events_a1b2c3d4');
    ...
    expect(normalizeTenantSlug('  Moon क Showcases  ')).toBe('moon-showcases');
  });
  ...
  it('keeps Enterprise as a superset and maps direct legacy calls to plan features', () => {
    ...
    expect(requiredFeatureForOperation('adminAiSearchTimelines')).toBe('visual_ai');
