...
const vendorServiceSchema = z.object({
  ...
  unit_type: z.enum(['per_person', 'per_room_per_night', 'per_vehicle', 'per_group', 'fixed']),
  net_cost: z.number().min(0),
  is_active: z.boolean().default(true),
  is_public: z.boolean().default(true),
});

export const adminGetVendorServices = defineOperation({ method: 'POST' })
  .validator(z.object({ auth: legacy.adminAuthSchema, vendorId: z.number() }))
  .handler(async ({ data }) => {
    await legacy.requireAdmin(data.auth);
    return await prisma.vendor_service_offerings.findMany({
      where: { vendor_id: data.vendorId },
      orderBy: [{ category: 'asc' }, { name: 'asc' }],
    });
  });

export const adminSaveVendorServices = defineOperation({ method: 'POST' })
  .validator(
    z.object({
      auth: legacy.adminAuthSchema,
      vendorId: z.number(),
      services: z.array(vendorServiceSchema),
    }),
  )
  .handler(async ({ data }) => {
    await legacy.requireAdmin(data.auth);
    await prisma.$transaction(async (tx) => {
      await tx.vendor_service_offerings.deleteMany({ where: { vendor_id: data.vendorId } });
      if (data.services.length) {
        await tx.vendor_service_offerings.createMany({
  ...
  .handler(async ({ data }) => {
    ...
    if (process.env.GEMINI_API_KEY) {
      try {
        ...
        const prompt = `You are an AI data extraction pipeline. A events vendor replied with their rates/catalog.
...
For packages: { title, days, location, b2b_price }
...
export const adminApproveInventoryDraft = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    if (draft.inventory_type === 'car') {
      ...
      await pool.query(
        `INSERT INTO packages (slug, title, days, location, vendor_id, b2b_price, price_inr, highlights, images)
         VALUES (?, ?, ?, ?, ?, ?, ?, '[]', '[]')`,
        [
          slug,
...
export const adminRenderRfqTemplate = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const vars = await legacy.buildRfqTemplateVars(
      data.packageId,
      data.eventDates,
      data.customVenues,
    );
