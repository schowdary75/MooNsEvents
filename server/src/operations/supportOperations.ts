...
export const getChatSmartReplies = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const prompt = `You are an expert events sales and support agent at MooNs Events (India). 
  ...
  .handler(async ({ data }): Promise<any[]> => {
    ...
    const [chats] = await pool.query(
      `SELECT c.*,
       ...
       LEFT JOIN global_chat_conversation_pins p
         ON BINARY p.target_id = BINARY CAST(c.id AS CHAR)
        AND p.target_type = 'customer'
        AND BINARY p.user_id = BINARY (SELECT CAST(id AS CHAR) FROM users WHERE email = ? LIMIT 1)
