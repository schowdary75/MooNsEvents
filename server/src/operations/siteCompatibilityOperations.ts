...
export const getCityCoordinates = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    for (const city of data.cities) {
      const cleaned = city
        .replace(/^(Arrival in|Departure from|Exploring|Showcase of|Day at|Visit to)\s+/i, '')
        .trim();
      ...
      try {
        const response = await fetch(
          ...
          {
            headers: { 'User-Agent': 'MooNs-EventApp/1.0' },
...
export const submitHolidayReview = defineOperation({ method: 'POST' })
  .validator(
    z.object({
      itemType: z.enum(['general', 'package', 'stay']),
...
export const getBookingDetailsWithRunOfShow = defineOperation({ method: 'POST' })
  .validator(
    z.object({
