...
import {
  publishEventInvalidation,
  type EventInvalidationReason,
} from '../services/eventRealtimeService.js';
import { getTenantRuntime } from '../config/tenantContext.js';
import { eventDayNumber as calculateEventDayNumber } from '../utils/eventTime.js';
...
function eventDayNumber(eventDate: Date): number {
  return calculateEventDayNumber(eventDate, getTenantRuntime()?.timezone ?? 'Asia/Kolkata');
}
...
async function publishBookingInvalidation(bookingId: number, reason: EventInvalidationReason) {
  const booking = await prisma.bookings.findUnique({
    where: { id: bookingId },
    select: { user_id: true },
  });
  if (booking) await publishEventInvalidation(booking.user_id, bookingId, reason);
}
...
export const adminGetJourneyBoard = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const events = bookings.map((booking) => {
      ...
      const dayNumber = eventDayNumber(booking.travel_date);
      ...
      return {
        ...
        location: booking.item_name,
        itemType: booking.item_type,
        eventDate: booking.travel_date,
        dayNumber,
        totalDays,
        phase,
        status,
...
export const adminGetEventDetail = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    return {
      booking: {
        ...
        dayNumber: eventDayNumber(booking.travel_date),
      },
      guest,
...
export const adminUpdateEventActivityStatus = defineOperation({ method: 'POST' })
  .validator(
    z.object({
...
export const adminUpdateEventMilestoneStatus = defineOperation({ method: 'POST' })
  .validator(
    z.object({
      auth: adminAuthSchema,
  ...
  .handler(async ({ data }) => {
    ...
    await logJourneyAdminAction(
      admin.email,
      'production.milestone_status',
      'event_live_milestone',
      milestone.id,
      { status: data.status },
    );
  ...
  .handler(async ({ data }) => {
    ...
    const rows = incidents.map((incident) => {
      ...
      return {
        ...
        location: booking?.item_name ?? null,
        guestName: guest?.name ?? 'Unknown guest',
        guestPhone: guest?.phone ?? null,
        guestEmail: guest?.email ?? null,
        issueType: incident.issue_type,
