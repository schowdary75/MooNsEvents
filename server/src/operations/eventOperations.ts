...
import {
  ensureCanonicalEventForBooking,
  ensureClientForContact,
  client360,
  eventWorkspace,
} from '../services/eventDomainService.js';
...
export const adminListClients = defineOperation({ method: 'POST' })
  .validator(
    z.object({
...
export const adminGetClient360 = defineOperation({ method: 'POST' })
  .validator(z.object({ auth: adminAuthSchema, clientId: z.string().uuid() }))
  .handler(async ({ data }) => {
    await requireLeadStaff(data.auth);
    return client360(data.clientId);
  });

export const adminGetClient360ByCrmClient = defineOperation({ method: 'POST' })
  .validator(z.object({ auth: adminAuthSchema, crmClientId: z.number().int().positive() }))
  .handler(async ({ data }) => {
    await requireLeadStaff(data.auth);
    const crmClient = await prisma.crm_clients.findUnique({ where: { id: data.crmClientId } });
    if (!crmClient) throw new Error('CRM client not found');
    ...
    return client360(client.id);
  });

export const adminGetEventWorkspace = defineOperation({ method: 'POST' })
  .validator(z.object({ auth: adminAuthSchema, eventId: z.string().uuid() }))
  .handler(async ({ data }) => {
    await requireLeadStaff(data.auth);
    return eventWorkspace(data.eventId);
  });
...
export const adminReviewMayaActionProposal = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    return reviewMayaAction({
      ...
      // through the authenticated /events-governance endpoint.
      recentMfa: false,
    });
  });

export const adminBackfillCanonicalEventDomain = defineOperation({ method: 'POST' })
  .validator(
  ...
  .handler(async ({ data }) => {
    ...
    for (const booking of bookings) {
      try {
        await ensureCanonicalEventForBooking(booking.id);
        migrated += 1;
      } catch (error) {
        failures.push({
    ...
    for (const refund of legacyRefunds) {
      ...
      const event = booking
        ? await ensureCanonicalEventForBooking(booking.id).catch(() => null)
        : null;
      const status =
      ...
      await prisma.canonicalRefundCase.create({
        data: {
          ...
          travellerId: event?.travellerId ?? null,
          status,
...
export const adminGetEventCapabilityReadiness = defineOperation({ method: 'POST' })
...
export const adminGetSupplierOperationsBoard = defineOperation({ method: 'POST' })
  ...
  .handler(async ({ data }) => {
    ...
    const events = await prisma.travelTrip.findMany({
    ...
    return reservations.map((reservation) => {
      ...
      return {
        reservation,
        service,
        event: events.find((row) => row.id === service?.tripId) ?? null,
        supplier: suppliers.find((row) => row.id === reservation.supplierId) ?? null,
      };
    });
  });

export const adminGetEventFinanceQueue = defineOperation({ method: 'POST' })
  .validator(z.object({ auth: adminAuthSchema }))
  .handler(async ({ data }) => {
    await requireLeadStaff(data.auth);
