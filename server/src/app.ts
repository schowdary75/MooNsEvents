...
import { botGuard } from './middlewares/botGuard.js';
import { ipBlocklist } from './middlewares/ipBlocklist.js';
...
export function createApp() {
  ...
  // --- Security layer 1: IP blocklist & bot guard ---
  app.use(ipBlocklist);
  app.use(botGuard);
  ...
  app.use(
    '/api/docs',
    swaggerUi.serve,
    swaggerUi.setup(openapi, { customSiteTitle: 'MooNsEvents API' }),
  );
