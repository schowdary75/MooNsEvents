...
This is the operational contract for the MooNsEvents commercial foundation.
...
- `moonsevents_platform` stores global identities, memberships, tenants,
...
The repository CI installs dependencies, generates both Prisma clients, checks
for committed secrets, verifies formatting, runs lint and typecheck, executes the
current test suites, and builds the client and server. Infrastructure validation,
database migration rehearsals, provider integration tests, representative load
tests, staged beta, restore drills, and independent tenant-escape penetration
testing remain external launch gates; the current CI workflow does not claim to
perform them.
