# MooNsEvents product tour

This tour uses direct Chrome captures from a locally running copy of MooNsEvents. The images are
not concept mock-ups and do not claim an unsubstantiated customer deployment. The lead workspace is filtered to a
...
2. Curate an editable run-of-show from the event brief or an inspiration image.
...
production segments for every day. AI generation is a starting point, not an irreversible answer.
...
SEO, media, run-of-show, route, supplier, and RFQ work.

## Supplier RFQs

![Supplier RFQ composer](images/product/vendor-rfq.png)

Operators choose package, venue, transport, or production scope; provide event dates; select suppliers;
