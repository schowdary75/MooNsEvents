...
export function parseCertificateArguments(arguments_) {
  ...
  if (!/^MOONSEVENTS-[A-Z]{2,8}-\d{4}-\d{4}$/.test(data.id)) {
    throw new Error('Certificate ID must match MOONSEVENTS-TYPE-YYYY-NNNN');
  }
  return data;
}
...
export function renderCertificate(data) {
  ...
  <title id="title">MooNsEvents ${safe.recognition} certificate for ${safe.displayName}</title>
  ...
  <text x="88" y="100" fill="#f8fafc" font-family="Inter, Segoe UI, sans-serif" font-size="30" font-weight="700">MooNsEvents</text>
  <text x="88" y="128" fill="#9fb2ca" font-family="Inter, Segoe UI, sans-serif" font-size="14" letter-spacing="2">OPEN event operating system</text>
  ...
  <text x="600" y="615" text-anchor="middle" fill="#718399" font-family="Inter, Segoe UI, sans-serif" font-size="12">Verify on github.com/schowdary75/moonsevents · @${safe.login} · ${safe.id}</text>
</svg>
`;
}
