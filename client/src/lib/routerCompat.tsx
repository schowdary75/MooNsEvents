...
function location({ to, params, search }: NavigateOptions) {
  let pathname = to;
...
export function useNavigate() {
  ...
  return (options: NavigateOptions | string | number) => {
    ...
    return navigate(location(options), { replace: options.replace });
  };
}
...
export function Link({ to, params, search, ...props }: CompatLinkProps) {
  return <ReactRouterLink to={location({ to, params, search })} {...props} />;
}
