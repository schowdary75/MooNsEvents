export const RFQ_SCOPE_ORDER = ['full', 'venues', 'photography', 'decoration', 'catering', 'organizer', 'production'] as const;

export type RfqScope = (typeof RFQ_SCOPE_ORDER)[number];

export interface RfqEventDateInput {
  from?: Date;
  to?: Date;
}

export interface RfqVenueRequestInput {
  name: string;
...
export interface BuildRfqPayloadInput {
  ...
  eventDates?: RfqEventDateInput;
  venueRequests?: readonly RfqVenueRequestInput[];
  vendorIds: readonly number[];
  message?: RfqMessageContent;
}

export interface RfqEventDates {
  from: string;
  to: string;
  label: string;
}

export interface RfqVenueRequest {
  name: string;
  source: 'catalog' | 'custom';
}
...
export interface RfqPayload {
  packageId: number;
  scope: RfqScope[];
  eventDates: RfqEventDates;
  venueRequests: RfqVenueRequest[];
  vendorIds: number[];
  message: RfqMessageContent;
}
...
export class RfqPayloadValidationError extends Error {
  constructor(
    public readonly field: 'packageId' | 'scope' | 'eventDates' | 'venues' | 'vendors' | 'message',
    message: string,
  ) {
    super(message);
...
function normalizeEventDates(input: RfqEventDateInput | undefined): RfqEventDates {
  if (!isValidDate(input?.from) || !isValidDate(input?.to)) {
    throw new RfqPayloadValidationError('eventDates', 'Select both events dates.');
  }
  if (input.to.getTime() < input.from.getTime()) {
    throw new RfqPayloadValidationError(
      'eventDates',
      'The events end date cannot be before the start date.',
    );
  }
...
function normalizeVenueRequests(requests: readonly RfqVenueRequestInput[] = []): RfqVenueRequest[] {
  const seen = new Set<string>();
  return requests.flatMap((request, index) => {
    if (!request || typeof request.name !== 'string') {
      throw new RfqPayloadValidationError(
        'venues',
        `Venue request ${index + 1} must include a name.`,
      );
    }
    ...
    if (!name || name.length > 160 || hasControlCharacter) {
      throw new RfqPayloadValidationError(
        'venues',
        `Venue request ${index + 1} has an invalid name.`,
      );
    }
...
export function buildRfqPayload(input: BuildRfqPayloadInput): RfqPayload {
  ...
  return {
    ...
    eventDates: normalizeEventDates(input.eventDates),
    venueRequests: normalizeVenueRequests(input.venueRequests),
...
export function toRfqComposeRequest(payload: RfqPayload): RfqComposeRequest {
  return {
    ...
    scope: payload.scope,
    eventDates: payload.eventDates.label,
    customVenues: payload.venueRequests.map((venue) => venue.name),
  };
}
