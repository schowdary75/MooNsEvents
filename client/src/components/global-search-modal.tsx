...
import { Search, X, Briefcase, Building2, ArrowRight, Loader2 } from 'lucide-react';
import { getPackages, getStays, PackageRow, StayRow } from '@/lib/api/db.functions';
...
export function GlobalSearchModal({ isOpen, onClose }: GlobalSearchModalProps) {
  ...
  useEffect(() => {
    ...
    const loadSearchData = async () => {
      setLoading(true);
      try {
        const [pkgs, stys] = await Promise.all([getPackages(), getStays()]);
        setPackages(pkgs);
        setStays(stys);
      } catch (err) {
  ...
  const matchedStays = cleanQuery
    ? stays.filter(
        (s) =>
          s.venue.toLowerCase().includes(cleanQuery) ||
          s.name.toLowerCase().includes(cleanQuery) ||
          s.country.toLowerCase().includes(cleanQuery),
      )
    : [];

  const totalResults = matchedPackages.length + matchedStays.length;
      ...
      <div className="relative z-10 w-full max-w-2xl bg-black border border-white/10 rounded-3xl overflow-hidden shadow-2xl animate-scaleUp text-left flex flex-col max-h-[80vh] font-sans">
        ...
        <div className="flex items-center gap-3 px-6 py-4 border-b border-white/10">
          ...
          <input
            ref={inputRef}
            type="text"
            placeholder="Search packages, boutique venues, or local activities..."
            value={query}
          ...
          {loading ? (
            ...
            <div className="text-center py-8 space-y-3">
              ...
              <p className="text-xs text-white/40 leading-relaxed max-w-md mx-auto">
                Type an event location or venue name to explore catalog offerings instantly.
              </p>
            </div>
          ) : totalResults === 0 ? (
            // No results found
            <div className="text-center py-12 space-y-2 text-white/40">
              ...
              <p className="text-xs leading-relaxed max-w-sm mx-auto">
                Try searching for a country name, venue keyword, or check your spelling.
              </p>
            </div>
          ) : (
            <div className="space-y-6">
              {/* Category 1: Slow-Events Packages */}
              {matchedPackages.length > 0 && (
                <div className="space-y-2.5">
                  <h3 className="text-[10px] uppercase tracking-[0.2em] font-semibold text-white/40 flex items-center gap-2">
                    <Briefcase size={12} />
                    <span>Events Packages ({matchedPackages.length})</span>
                  </h3>
        ...
        <div className="px-6 py-3 border-t border-white/10 bg-white/[0.02] flex justify-between items-center text-[10px] text-white/30 font-sans">
          <span>Search matches dynamically across all slow events products</span>
