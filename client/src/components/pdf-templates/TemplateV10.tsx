...
const s = StyleSheet.create({
  pageTeal: { padding: 0, fontFamily: 'Inter', backgroundColor: C.forest },
  ...
  footer: {
    ...
    paddingTop: 10,
    borderTopWidth: 1,
    borderTopColor: '#1B6B54',
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
  },
  footerLight: { borderTopColor: C.cardBorder },
  footerText: { fontSize: 7, color: '#5B8D7E', letterSpacing: 0.5 },
  footerPage: { fontSize: 7, color: C.teal, fontWeight: 600, letterSpacing: 1 },
  accentLine: { height: 2, backgroundColor: C.teal },
  ...
  editionBadgeText: { color: C.tealLight, fontSize: 7, letterSpacing: 3, fontWeight: 600 },
  coverTitle: {
    color: C.white,
    fontSize: 30,
    fontWeight: 600,
  ...
  coverDesc: { color: '#A7F3D0', fontSize: 11, lineHeight: 1.6, marginBottom: 24 },
  metaRow: { flexDirection: 'row', gap: 8 },
  metaBox: { flex: 1, borderWidth: 1, borderColor: '#1B6B54', borderStyle: 'solid', padding: 10 },
  metaLabel: { color: C.tealLight, fontSize: 7, letterSpacing: 2, marginBottom: 4 },
  metaValue: { color: C.white, fontSize: 11, fontWeight: 600 },
  priceBox: { backgroundColor: C.tealDark, borderRadius: 6, padding: 18, marginTop: 20 },
  priceLabel: { color: C.tealLight, fontSize: 8, letterSpacing: 2, marginBottom: 6 },
  priceLg: { color: C.white, fontSize: 26, fontWeight: 600 },
  ...
  familyBadgeText: { color: C.amber, fontSize: 7, fontWeight: 600, letterSpacing: 0.5 },
  ...
  incBadgeText: { color: C.teal, fontSize: 7, fontWeight: 600, letterSpacing: 1 },
  ...
  incNumText: { color: C.tealDark, fontSize: 7, fontWeight: 600 },
  incItemText: { flex: 1, color: C.textMid, fontSize: 9, lineHeight: 1.5 },

  // Investment
  investCard: { backgroundColor: '#1B6B54', borderRadius: 6, padding: 14, marginBottom: 10 },
  investLabel: {
    color: C.tealLight,
    fontSize: 7,
    letterSpacing: 2,
  ...
  investRowDesc: { color: '#5B8D7E', fontSize: 8 },
  investRowPrice: { color: C.tealLight, fontSize: 10, fontWeight: 600 },
  investGrand: { backgroundColor: C.teal, borderRadius: 6, padding: 16, marginTop: 6 },
  investGrandLabel: {
    color: C.forest,
    fontSize: 7,
    letterSpacing: 2,
  ...
  termsItemNumText: { color: C.tealDark, fontSize: 7, fontWeight: 600 },
...
const Footer = ({
  ...
  <View style={[s.footer, !dark ? s.footerLight : {}]} fixed>
    <Text style={s.footerText}>MooNs · Family Collection</Text>
    <Text
      style={s.footerPage}
...
export const TemplateV10 = ({
  ...
  return (
    <Document>
      <Page size="A4" style={s.pageTeal}>
        <View style={[s.pad, { paddingTop: 40, paddingBottom: 60 }]}>
          ...
          <View style={s.editionBadge}>
            <Text style={s.editionBadgeText}>FAMILY PLAN</Text>
          </View>
          <Text style={s.coverTitle}>{packageName || 'Family Adventure'}</Text>
          <Text style={s.coverDesc}>
            {leadNotes ||
              'A wonderful family escape tailored for all ages — from kids to grandparents.'}
          </Text>
          <View style={[s.accentLine, { width: 40, marginBottom: 16 }]} />
          <View style={s.metaRow}>
            <View style={s.metaBox}>
              <Text style={s.metaLabel}>LOCATION</Text>
              <Text style={s.metaValue}>{leadLocation || 'Open'}</Text>
            </View>
            <View style={s.metaBox}>
            ...
            <View style={s.metaBox}>
              <Text style={s.metaLabel}>ADULTS</Text>
          ...
          <View style={s.priceBox}>
            <Text style={s.priceLabel}>FAMILY TOTAL INVESTMENT</Text>
      ...
      {runOfShow.length > 0 && (
        <Page size="A4" style={s.pageLight}>
          <View style={[s.pad, { paddingBottom: 60 }]}>
            <Text style={s.sectionSub}>FAMILY ADVENTURE</Text>
            ...
            {runOfShow.map((d) => (
              <View key={d.day_number} style={s.itinCard} wrap={false}>
                ...
                <View style={s.familyBadge}>
                  <Text style={s.familyBadgeText}>KID-FRIENDLY</Text>
                </View>
              </View>
            ))}
          </View>
          ...
          <View style={[s.pad, { paddingBottom: 60 }]}>
            ...
            {stays && stays.length > 0 && (
              <View style={{ marginBottom: 20 }}>
                <Text style={{ fontSize: 9, fontWeight: 600, color: C.tealDark, marginBottom: 10 }}>
      ...
      <Page size="A4" style={s.pageLight}>
        <View style={[s.pad, { paddingBottom: 60 }]}>
          <Text style={s.sectionSub}>FAMILY KIT</Text>
      ...
      <Page size="A4" style={s.pageTeal}>
        <View style={[s.pad, { paddingBottom: 60 }]}>
          <Text style={{ color: C.tealLight, fontSize: 8, letterSpacing: 3, marginBottom: 6 }}>
            FAMILY INVESTMENT
          </Text>
          ...
          <View style={s.investCard}>
            ...
            <View style={s.investRow}>
              <View style={{ flex: 1 }}>
                <Text style={s.investRowTitle}>{packageName || 'Family Package'}</Text>
                <Text style={s.investRowDesc}>All-inclusive family experience</Text>
              </View>
          ...
          {activitiesCost > 0 && (
            <View style={s.investCard}>
              <Text style={s.investLabel}>FAMILY ACTIVITIES</Text>
              {activities.map((a) => (
          ...
          <View style={s.investGrand}>
            <Text style={s.investGrandLabel}>FAMILY TOTAL</Text>
      ...
      <Page size="A4" style={s.pageLight}>
        <View style={[s.pad, { paddingBottom: 60 }]}>
          ...
          <View style={s.termsSection}>
            ...
            {[
              '30+ days: Full refund minus INR 3,000 per person.',
          ...
          <View style={s.termsSection}>
            ...
            {[
              ...
              'Child pricing applies to ages 2–11 with valid ID.',
              'Only runOfShow-listed services included.',
              'Twin/eventle-sharing accommodations unless specified.',
            ].map((t, i) => (
              <View key={i} style={s.termsItem}>
                <View style={s.termsItemNum}>
                  <Text style={s.termsItemNumText}>{i + 1}</Text>
