...
import { Sunrise, BookOpen, Package, Building2, Camera, Palette, Utensils, Briefcase, SlidersHorizontal, CalendarIcon, Plus, X } from 'lucide-react';
...
const SCOPE_OPTIONS = [
  {
    ...
    label: 'Full Package',
    icon: Package,
    description: 'RunOfShow, activities, inclusions & exclusions',
  },
  {
    key: 'venues',
    label: 'Venues',
  ...
  },
  {
    ...
    label: 'Photography',
    icon: Camera,
    description: 'Coverage, crew, deliverables and usage terms',
  },
  {
    key: 'decoration',
    label: 'Decoration',
    icon: Palette,
    description: 'Theme, stage, floral and installation pricing',
  },
  {
    key: 'catering',
    label: 'Catering & Hospitality',
    icon: Utensils,
    description: 'Menu, guest count, service staff and hospitality',
  },
  {
    key: 'organizer',
    label: 'Event Organizer',
    icon: Briefcase,
    description: 'Planning, coordination and event-day management',
  },
  {
    key: 'production',
    label: 'Production & AV',
    icon: SlidersHorizontal,
    description: 'Stage, sound, lighting, LED and technical crew',
  },
] as const;
...
export function SendRfqModal({
  ...
  // New State for DateRange and Venues
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (masterStays.length === 0 && auth) {
        // Fetch venues to let the user select from list
        getAccommodationListings()
          .then((res: any[]) => {
            const dest = (location || country || '').toLowerCase();
            const filtered = res.filter(
              (item) =>
                (item.location || '').toLowerCase() === dest ||
  ...
  }, [isOpen, auth, vendors.length, location, country]);
  ...
  const buildCurrentRfqPayload = (message?: RfqMessageContent): RfqPayload => {
    return buildRfqPayload({
      ...
      venueRequests: selectedVenues.map((name) => ({ name })),
  ...
  const addCustomVenue = () => {
    if (customVenueInput.trim() && !selectedVenues.includes(customVenueInput.trim())) {
      setSelectedVenues((prev) => [...prev, customVenueInput.trim()]);
    }
    setCustomVenueInput('');
  };

  const removeVenue = (venue: string) => {
    setSelectedVenues((prev) => prev.filter((h) => h !== venue));
  };
            ...
            <>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div>
                    <Label className="text-base text-red-500">Events Dates (Required)</Label>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button
                          ...
                          className={cn(
                            ...
                            !eventDateRange && 'text-muted-foreground border-red-200 bg-red-50',
                          )}
                        >
                          ...
                          {eventDateRange?.from ? (
                            eventDateRange.to ? (
                              <>
                                {format(eventDateRange.from, 'LLL dd, y')} -{' '}
                                {format(eventDateRange.to, 'LLL dd, y')}
                              </>
                            ) : (
                              format(eventDateRange.from, 'LLL dd, y')
                            )
                          ) : (
                            <span>Select events dates</span>
                          )}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0" align="start">
                        <Calendar
                          initialFocus
                          mode="range"
                          defaultMonth={eventDateRange?.from}
                  ...
                  {(rfqScope.includes('venues') || rfqScope.includes('full')) && (
                    <div className="p-3 bg-muted/30 border rounded-lg space-y-3">
                      <div>
                        <Label className="text-sm">Specific Venues for Quote (Optional)</Label>
                        <p className="text-xs text-muted-foreground mb-2">
                          Select or add specific venues. If provided, these will override the
                          default package venues in the RFQ email.
                        </p>

                        <div className="flex gap-2">
                          <Select
                            onValueChange={(val) => {
                              if (!selectedVenues.includes(val))
                                setSelectedVenues([...selectedVenues, val]);
                            }}
                          >
                            ...
                            <SelectContent>
                              {masterStays.length > 0 ? (
                                ...
                                <SelectItem value="none" disabled>
                                  No venues in catalog for this location
                                </SelectItem>
                              )}
                            </SelectContent>
                          </Select>
                        ...
                        <div className="flex gap-2 mt-2">
                          ...
                          />
                          <Button variant="secondary" onClick={addCustomVenue} type="button">
                            Add
                          </Button>
                        </div>
                ...
                <div className="space-y-3 flex flex-col h-full">
                  <Label className="text-base">
                    2. Select Vendors in {location || country || 'Location'}
                  </Label>
                  ...
                  <div className="flex-1 overflow-y-auto rounded-md border p-2 space-y-2 max-h-[60vh]">
                    {vendors
                      .filter(
                        (v) =>
                          (v.coverage_areas || '').includes(location || '') ||
        ...
        <div className="p-4 border-t flex justify-end gap-3 bg-muted/30">
          ...
          {!rfqPreview ? (
            <Button
              ...
              disabled={
                rfqSelectedVendors.length === 0 || !eventDateRange?.from || !eventDateRange?.to
              }
            >
              Next: Compose Email
            </Button>
