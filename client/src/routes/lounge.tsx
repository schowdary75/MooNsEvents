...
function ClientLoungePage() {
  ...
  const loadData = async () => {
    try {
      ...
      toast.error('Could not load runOfShow. Please verify your link.');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#F9F8F5] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-xs text-muted-foreground mt-1">Fetching your bespoke events runOfShow</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-[#F9F8F5] flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-sm text-muted-foreground mt-2 max-w-md">
          The runOfShow link you accessed might have expired or is invalid. Please contact your
          events advisor.
        </p>
      </div>
    );
              ...
              <div className="absolute bottom-6 left-6 right-6 text-white space-y-2">
                ...
                <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-gray-300">
                  <span className="flex items-center gap-1">
                    <MapPin className="w-3.5 h-3.5 text-primary" /> {pkg.location}, {pkg.country}
                  </span>
                  <span className="flex items-center gap-1">
                    ...
                    {activeDayComment === day.day_number && (
                      <div className="mt-3 bg-muted/40 p-4 rounded-lg border border-border/60 space-y-4">
                        <h5 className="text-[11px] font-bold uppercase tracking-wider text-muted-foreground">
                          RunOfShow Collaborative Feedback
                        </h5>
          ...
          <div className="p-6 rounded-2xl border bg-card shadow-sm space-y-4">
            ...
            <div className="border-b pb-4 space-y-1">
              ...
              <span className="text-[10px] text-muted-foreground block">
                Includes VIP transfers, venues, experiences, and local DMCs.
              </span>
            </div>
          ...
          <div className="bg-card w-full max-w-md rounded-2xl border border-border/80 p-6 shadow-2xl space-y-4">
            <div className="flex justify-between items-start">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  Sign and approve your custom MooN runOfShow.
                </p>
              </div>
              <Button
                variant="ghost"
            ...
            <div className="space-y-3.5">
              ...
              <p className="text-[10px] text-muted-foreground leading-relaxed">
                This preview cannot record acceptance. Continue to the secure client event portal to
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-[#F9F8F5] dark:bg-background">
              {generalComments.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center text-center space-y-3 opacity-60">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Send a message to your dedicated events planner to request changes or ask
