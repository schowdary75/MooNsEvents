...
function QuoteCard({ quote, client, reload }) {
  const [signerName, setSignerName] = useState(client.displayName ?? '');
        ...
        <div className="mt-4 space-y-2 rounded-xl bg-slate-50 p-3">
          {quote.comments.map((item) => (
            <p key={item.id} className="text-sm text-slate-700">
              <b>{item.authorType === 'client' ? 'You' : 'Planner'}:</b> {item.body}
            </p>
          ))}
        </div>
      )}
...
function DocumentUploader({ events, reload }) {
  ...
  return (
    <div className="mt-4 rounded-2xl border border-dashed border-sky-300 bg-sky-50 p-4">
      ...
      <div className="mt-3 grid gap-2 md:grid-cols-4">
        ...
        >
          {['credential', 'permit', 'id', 'insurance', 'ticket', 'voucher', 'medical', 'other'].map(
            (type) => (
    ...
    try {
      const { data } = await customerClient.post(
        `/customer/bookings/${recovery.bookingId}/incidents/${recovery.incidentId}/receipts/presign`,
        {
          ...
          mimeType: file.type,
          sizeBytes: file.size,
          expenseType: recovery.issueType === 'venue_issue' ? 'venue' : 'transport',
...
function Hub({ data, reload, logout }) {
  const futureEvents = useMemo(() => data.events ?? [], [data.events]);
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900">
      <header className="sticky top-0 z-20 border-b border-slate-200 bg-white/90 backdrop-blur">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-3">
          <div>
            <div className="text-lg font-black">MooNs client event portal</div>
            <div className="text-xs text-slate-500">Welcome, {data.client.displayName}</div>
          </div>
      ...
      <main className="mx-auto max-w-6xl space-y-8 px-4 py-7">
          ...
          <div className="grid gap-5 md:grid-cols-[1fr_auto] md:items-center">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black">
                Events, payments and events documents in one wallet.
              </h1>
                ...
                >
                  <div className="flex flex-wrap justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-sm text-slate-500">
                        ...
                        {event.location ?? 'Custom production'} ·{' '}
                    ...
                    <div className="text-right text-xs text-slate-500">
                      Reference
                      <br />
                      <b className="text-slate-900">{event.reference}</b>
                    </div>
                  </div>
                  <div className="mt-4">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-3">
                    <div className="rounded-xl bg-slate-50 p-3 text-sm">
                      ...
                      {event.party.length} participant(s)
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3 text-sm">
                      <Plane className="mr-2 inline h-4 w-4 text-sky-700" />
                      {event.services.length} service(s)
                    </div>
                    <div className="rounded-xl bg-slate-50 p-3 text-sm">
                      <WalletCards className="mr-2 inline h-4 w-4 text-sky-700" />
                      {event.paymentSchedule.length} instalment(s)
                    </div>
                  </div>
                  {(event.recoveries ?? []).map((recovery) => (
        ...
        <section>
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="text-xl font-black">Events wallet</h2>
          </div>
          ...
          <DocumentUploader events={data.events} reload={reload} />
        </section>
        ...
        <section className="rounded-2xl border border-rose-200 bg-rose-50 p-5">
          ...
          <p className="mt-1 text-sm text-rose-800">
            ...
            chat and write “emergency” or “lost credential” for immediate human escalation. Save your
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gradient-to-br from-sky-950 to-slate-950 p-4">
      <form onSubmit={login} className="w-full max-w-md rounded-3xl bg-white p-7 shadow-2xl">
        ...
        <h1 className="mt-2 text-3xl font-black">client event portal</h1>
        <p className="mt-2 text-sm text-slate-500">
          Sign in to view events, proposals, documents, payments and live support.
        </p>
