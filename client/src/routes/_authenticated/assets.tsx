...
type ServiceKey = 'packages' | 'stays' | 'cars';
type GalleryItem = {
  id: string;
  url: string;
  title: string;
...
function Assets() {
  ...
  const isLoading =
    packagesQ.isLoading || carsQ.isLoading || staysQ.isLoading;
  const isError = packagesQ.isError && carsQ.isError && staysQ.isError;
  ...
  const allItems = useMemo<GalleryItem[]>(() => {
    ...
    return out;
  }, [packagesQ.data, staysQ.data, carsQ.data]);
  ...
  return (
    <div className="space-y-6 p-6 lg:p-8">
      ...
      <div className="flex flex-wrap items-center gap-2">
        {(['all', 'packages', 'stays', 'cars'] as const).map((key) => (
          <button
            key={key}
            type="button"
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border border-dashed border-border bg-card p-10 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Add photos to approved event services or packages and they will appear here.
          </p>
        </div>
      ) : view === 'flow' ? (
