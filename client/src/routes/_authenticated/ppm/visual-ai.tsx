...
import { adminAiVisualScrapbook, adminSaveAiRunOfShowToActivities } from '@/lib/api/db.functions';
...
function ScrapbooksPage() {
  ...
  const uploadInTimelineRef = useRef(false);
  ...
  const handleSaveToCatalog = async () => {
    if (!result || !result.runOfShow || !result.location) return;
    setIsSaving(true);
    try {
      await adminSaveAiRunOfShowToActivities({
        data: {
          location: result.location,
          runOfShow: result.runOfShow,
        },
      });
      toast.success('Saved runOfShow to Activities catalog!');
    } catch (err) {
  ...
  const handleImageUpload = async (file: File) => {
    if (uploadInTimelineRef.current) return;
    ...
    uploadInTimelineRef.current = true;
    ...
    try {
      ...
      try {
        ...
        setResult(res);
        toast.success('AI analyzed the image and built an runOfShow!');
      } catch (err) {
        const message =
      ...
      uploadInTimelineRef.current = false;
            ...
            {isGenerating ? (
              <div className="h-full flex flex-col items-center justify-center text-muted-foreground">
                ...
                <p className="text-xs mt-2 text-primary/70">
                  Identifying vibes & drafting runOfShows
                </p>
              </div>
            ) : result ? (
              <div className="space-y-6 animate-slide-up">
                ...
                <div>
                  <h4 className="font-bold text-sm uppercase tracking-wider text-muted-foreground mb-4">
                    Sample 3-Day RunOfShow
                  </h4>
                  <div className="space-y-4">
                    {result.runOfShow?.map((day: any, idx: number) => (
                      <div
                        key={idx}
