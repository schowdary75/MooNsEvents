...
type VendorServiceDraft = {
  category: (typeof serviceOptions)[number];
  name: string;
  description: string;
  unit_type: 'per_person' | 'per_room_per_night' | 'per_vehicle' | 'per_group' | 'fixed';
  net_cost: number;
  is_active: boolean;
  is_public: boolean;
};
...
function Vendors() {
  ...
  const [activeRegion, setActiveRegion] = useState<RegionTab>('telangana');
  const [editing, setEditing] = useState(false);
  const [editingVendorId, setEditingVendorId] = useState<number | null>(null);
  const [isUploading, setIsUploading] = useState(false);
  const [vendorServices, setVendorServices] = useState<VendorServiceDraft[]>([]);
  ...
  const [form, setForm] = useState({
    ...
    phone: '',
    whatsapp: '',
    serviceCategories: ['venue'] as string[],
    coverageAreas: '',
    address: '',
    websiteUrl: '',
    bio: '',
  ...
  const handleEditVendor = async (vendor: VendorProfile) => {
    ...
    setForm({
      ...
      serviceCategories: (parsedServices || ['venue']) as any,
    ...
    if (auth) {
      try {
        const services = await adminGetVendorServices({ data: { auth, vendorId: vendor.id } });
        setVendorServices(
          (services || []).map((service: any) => ({
            ...
            net_cost: Number(service.net_cost || 0),
            is_active: Boolean(service.is_active),
            is_public: Boolean(service.is_public),
          })),
        );
      } catch {
  ...
  const fetchVendors = useCallback(() => {
    ...
    const request = (async () => {
      ...
      const [vendorsResult, queueResult, communicationsResult, draftsResult, templatesResult] =
        results;

      if (vendorsResult.status === 'fulfilled') setVendors(vendorsResult.value);
      else toast.error('Failed to load vendor directory');

      if (queueResult.status === 'fulfilled') setQueue(queueResult.value);
      if (communicationsResult.status === 'fulfilled') {
        const communicationsPayload = communicationsResult.value as any;
        setCommunications(communicationsPayload.data || communicationsPayload);
        setTotalCommunications(communicationsPayload.total || 0);
      }
      ...
      if (results.slice(1).some((result) => result.status === 'rejected')) {
        toast.error('Some vendor outreach tools could not be loaded');
      }
      setLoading(false);
    })();
        ...
        .map((r) => {
          return {
            ...
            service_categories: (r['Service Type'] || '')
              ...
              .map((s: string) => s.trim())
              .filter(Boolean),
            coverage_areas: r['Locations'] || r['Coverage'] || '',
  ...
  async function saveVendor() {
    try {
      if (editingVendorId && auth) {
        ...
        await adminSaveVendorServices({
          data: { auth, vendorId: editingVendorId, services: vendorServices },
        });
  ...
  const filteredVendors = vendors.filter((vendor) => {
    const matchesRegion = coverageMatchesRegion(
      ...
      {editing && (
        <div className="rounded-md border bg-background p-4">
          <div className="grid gap-3 md:grid-cols-3">
            ...
            />
            <Input
              placeholder="Coverage areas e.g. Hyderabad, Telangana"
          ...
          {editingVendorId ? (
            <div className="mt-4 rounded-md border p-3">
              <div className="mb-3 flex items-center justify-between gap-3">
                <div>
                  <h3 className="text-sm font-semibold">Vendor Service Catalog</h3>
                  <p className="text-xs text-muted-foreground">
                    These priced services become selectable in Package Builder and visible on the
                    website.
                  </p>
                </div>
                <Button
                  ...
                  onClick={() =>
                    setVendorServices((current) => [
                      ...current,
                      {
                        category: form.serviceCategories[0] || 'venue',
                        name: '',
                        description: '',
                        unit_type: 'fixed',
                        net_cost: 0,
              ...
              <div className="space-y-2">
                {vendorServices.map((service, index) => (
                  <div
                    key={index}
                    className="grid gap-2 rounded-md border bg-muted/20 p-2 lg:grid-cols-[140px_1fr_160px_140px_auto]"
                  >
                    <select
                        ...
                        setVendorServices((current) =>
                          current.map((item, itemIndex) =>
                            itemIndex === index
                              ? {
                                  ...item,
                                  category: event.target.value as VendorServiceDraft['category'],
                                }
                              : item,
                          ),
                        )
                        ...
                        setVendorServices((current) =>
                          current.map((item, itemIndex) =>
                            itemIndex === index
                              ? {
                                  ...item,
                                  unit_type: event.target.value as VendorServiceDraft['unit_type'],
                                }
                              : item,
                          ),
                        )
                    ...
                    <Input
                      ...
                      placeholder="Vendor net INR"
                      onChange={(event) =>
                        setVendorServices((current) =>
                          current.map((item, itemIndex) =>
                            itemIndex === index
                              ? { ...item, net_cost: Number(event.target.value) }
                              : item,
                          ),
                        )
                      }
                ...
                {vendorServices.length === 0 && (
                  <p className="py-4 text-center text-xs text-muted-foreground">
                    No services recorded. Add each service and its confirmed vendor net price.
                  </p>
                )}
              </div>
            </div>
            ...
            <p className="mt-3 text-xs text-muted-foreground">
              Save the vendor first, then edit it to add its individual priced services.
            </p>
          )}
              ...
              <TableBody>
                {paginatedItems.map((vendor) => (
                  <TableRow key={vendor.id}>
                    ...
                    <TableCell>
                      <div>{vendor.contact_name || 'Booking team'}</div>
                      <div className="text-xs text-muted-foreground">
                        {vendor.email || 'No public email'} · {vendor.phone || 'No public phone'}
                      </div>
                      {vendor.address && (
