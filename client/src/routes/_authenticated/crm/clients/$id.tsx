...
import { adminGetClient360ByCrmClient } from '@/lib/api/operations';
...
function ClientProfilePage() {
  ...
  const { data: profile, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [client, client360] = await Promise.all([
        adminGetClientById({ data: { auth, id: Number(id) } }),
        adminGetClient360ByCrmClient({ data: { auth, crmClientId: Number(id) } }),
      ]);
      return { client, client360 };
    },
    enabled: !!user && !!id,
  });
  ...
  const { client, client360 } = profile;
  const events = client360?.events ?? [];
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-lg bg-card border p-6 shadow-sm">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6">
            ...
            <div>
              <p className="text-xs text-zinc-500 font-medium uppercase mb-1">Last Event</p>
              <p className="text-sm font-medium text-zinc-700">
                {events[0]?.name ?? client.lastEvent ?? 'No event yet'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-card border shadow-sm rounded-lg p-6">
              ...
              <div className="p-8 text-center text-muted-foreground border border-dashed rounded-lg bg-muted/20">
                {client360?.openActions?.length ?? 0} Maya/staff action(s) need attention.
              </div>
            </div>
          </div>
          ...
          <div className="space-y-6">
            <div className="bg-card border shadow-sm rounded-lg p-6">
              <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
                <ShieldCheck className="h-5 w-5 text-emerald-600" /> Client readiness
              </h2>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span>Party identities</span>
                  <b>{client360?.identities?.length ?? 0}</b>
                </div>
                <div className="flex justify-between">
                  <span>Preferences</span>
                  <b>{client360?.preferences?.length ?? 0}</b>
                </div>
                <div className="flex justify-between">
                  <span>Consents</span>
                  <b>{client360?.consents?.filter((c) => c.status === 'granted').length ?? 0}</b>
                </div>
                <div className="flex justify-between">
                  <span>Secure documents</span>
                  <b>{client360?.documents?.length ?? 0}</b>
                </div>
              </div>
            </div>
            <div className="bg-card border shadow-sm rounded-lg p-6">
