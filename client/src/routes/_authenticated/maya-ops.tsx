...
// Allowed next steps mirror the server-side permit lifecycle.
...
function MayaOpsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [center, actions, readiness, suppliers, finance, killSwitches] = await Promise.all([
        ...
        adminGetEventCapabilityReadiness({ data: { auth } }),
        adminGetSupplierOperationsBoard({ data: { auth } }),
        adminGetEventFinanceQueue({ data: { auth } }),
        ...
        apiClient
          .get('/events-governance/kill-switches')
          .then(({ data }) => data.data)
          .catch(() => []),
      ]);
  ...
  async function advancePermit(caseId: number, to: string) {
    if (!auth) return toast.error('Your session is missing. Please sign in again.');
    try {
      const res = await adminAdvancePermitCase({ data: { auth, caseId, to } });
      if (res?.ok === false) return toast.error(res.error || 'Could not update permit case');
      toast.success(`Permit case moved to ${to.replace(/_/g, ' ')}`);
      await load();
    } catch (err) {
  ...
  async function reviewAction(proposal: any, decision: 'approve' | 'reject') {
    ...
    try {
      ...
      if (proposal.riskClass === 'high_risk') {
        await apiClient.post(`/events-governance/actions/${proposal.id}/review`, {
          decision,
          reason,
        });
      } else {
  ...
  async function viewIncidentReceipt(proposal: any) {
    try {
      const { data } = await apiClient.get(
        `/events-governance/actions/${proposal.id}/incident-receipt`,
      );
  ...
  async function toggleKillSwitch(item: any) {
    ...
    try {
      await apiClient.put('/events-governance/kill-switches', {
        scope,
        key,
  ...
  const timelineWatches = data?.timelineWatches ?? { active: 0, upcoming: [] };
  const permitCases = data?.permitCases ?? { total: 0, atRisk: 0, rows: [] };
  ...
  return (
    <div className="animate-fade-in space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Proactive client care — refunds, disruptions, timelines and permits Maya is watching.
          </p>
        </div>
      ...
      <div className="grid gap-3 md:grid-cols-3 xl:grid-cols-6">
        ...
        />
        <MetricCard
          label="Timelines watched"
          value={timelineWatches.active}
          icon={PlaneTakeoff}
          tone="bg-blue-500/10 text-blue-500"
          delay={120}
        />
        <MetricCard
          label="Permits at risk"
          value={permitCases.atRisk}
          icon={Stamp}
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <Panel title="Maya approval queue" icon={Bot}>
          ...
          <div className="border-t border-border/30 p-3 text-xs text-muted-foreground">
            Maya is <b>{readiness.maya?.policyMode ?? 'read only'}</b>. Timeline status:{' '}
            <b>{readiness.providers?.timelineStatus?.configured ? 'configured' : 'not configured'}</b>
            ; events rules:{' '}
            <b>
              {readiness.providers?.eventRules?.configured
                ? readiness.providers.eventRules.provider
        ...
        <Panel title="Supplier operations" icon={Building2}>
          <Table>
            <TableHeader>
              <TableRow className="border-border/30">
                <TableHead className="text-[11px] font-bold uppercase">Event / service</TableHead>
                ...
                suppliers.slice(0, 12).map((item: any) => (
                  <TableRow key={item.reservation.id}>
                    <TableCell>
                      ...
                      <div className="text-[11px] text-muted-foreground">
                        {item.event?.reference ?? 'Unlinked event'}
                      </div>
                    </TableCell>
        ...
        <Panel title="Events finance queue" icon={Landmark}>
          <div className="grid grid-cols-2 gap-3 p-4">
            {[
              ['Client receivables', finance.receivables.length],
        ...
        <Panel title="Permit cases" icon={Stamp}>
          <Table>
            <TableHeader>
              <TableRow className="border-border/30">
            ...
            <TableBody>
              {permitCases.rows.length === 0 ? (
                ...
                permitCases.rows.slice(0, 12).map((v: any) => (
                  <TableRow key={v.id} className="hover:bg-muted/30">
                      ...
                      <div className="flex items-center gap-1.5 text-sm font-semibold">
                        ...
                        {v.atRisk && (
                          <AlertTriangle
                            ...
                            title="At risk — events is near"
                          />
                        )}
                      </div>
                      <div className="text-[11px] text-muted-foreground">
                        {new Date(v.eventDate).toLocaleDateString('en-IN')}
                      </div>
                    </TableCell>
                    <TableCell className="text-xs capitalize">
                    ...
                    <TableCell className="text-right">
                      <div className="flex flex-wrap justify-end gap-1">
                        {(PERMIT_NEXT[v.status] || []).map((to: string) => (
                          <Button
                            key={to}
                            variant="ghost"
                            size="sm"
                            className="h-6 px-2 text-[10px] capitalize hover:bg-primary/10"
                            onClick={() => advancePermit(v.id, to)}
                          >
                            {to.replace(/_/g, ' ')}
                          </Button>
                        ))}
                        {(PERMIT_NEXT[v.status] || []).length === 0 && (
