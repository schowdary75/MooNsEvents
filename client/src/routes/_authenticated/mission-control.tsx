      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        <QueueCard
          ...
          items={(mc?.attentionQueues.followups || []).map((f: any) => ({
            id: f.id,
            primary: f.lead_name,
            secondary: `${f.follow_up_type} · ${f.location || 'Open'} · due ${timeAgo(f.follow_up_date)}`,
          }))}
        />
        <QueueCard
        ...
        <QueueCard
          ...
          items={(mc?.attentionQueues.newLeads || []).map((l: any) => ({
            id: l.id,
            primary: l.name,
            secondary: `${l.location || 'Open location'} · ${l.budget_range || 'Budget TBD'} · ${timeAgo(l.created_at)}`,
          }))}
        />
      </div>
