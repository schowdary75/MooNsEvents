    ...
    <div className="space-y-6">
      <div className="space-y-6">
        ...
        <div className="bg-card rounded-lg border shadow-sm overflow-hidden mb-8">
          ...
          {(['events', 'system'] as const).map((category) => {
...
const AUTOMATION_TEMPLATES: AutomationTemplate[] = [
  // ----------------------- Events business playbooks -----------------------
  {
    ...
    icon: UserPlus,
    description:
      'Greet a fresh enquiry instantly, share top locations, and hand a warm follow-up task to a events consultant.',
    steps: [
  ...
  },
  {
    ...
    icon: FileText,
    description:
      'Nudge clients who received an runOfShow quote but have not booked, across a 5-day sequence.',
    steps: [
      { order: 1, type: 'delay', detail: 'Wait 1 day' },
      { order: 2, type: 'email', detail: 'Reminder: "Your runOfShow is still available"' },
  ...
  },
  {
    ...
    icon: ShoppingCart,
    description:
      'Recover clients who started a booking but dropped off before payment, with a gentle incentive.',
    steps: [
      { order: 1, type: 'delay', detail: 'Wait 1 hour' },
      { order: 2, type: 'email', detail: 'Resume booking link + event summary' },
  ...
  },
  {
    ...
    name: 'Booking Confirmation & Event Prep',
    triggerEvent: 'booking.confirmed',
    category: 'events',
    icon: Plane,
    description:
      'Confirm the booking, deliver documents, and start pre-event preparation once payment succeeds.',
    steps: [
      { order: 1, type: 'email', detail: 'Confirmation + e-tickets / vouchers' },
      { order: 2, type: 'sms', detail: 'SMS: booking reference + support number' },
      { order: 3, type: 'task', detail: 'Ops: verify permit / documents checklist' },
    ],
  },
  {
    key: 'pre-departure',
    ...
    icon: BellRing,
    description:
      'Countdown reminders before the events date: packing tips, weather, check-in and pickup details.',
    steps: [
      { order: 1, type: 'email', detail: '7 days before: packing & weather guide' },
      { order: 2, type: 'delay', detail: 'Wait until 2 days before events' },
      { order: 3, type: 'whatsapp', detail: 'Pickup, check-in and emergency contacts' },
    ],
  },
  {
    key: 'post-event-review',
    name: 'Post-Event Review Request',
    triggerEvent: 'event.completed',
    category: 'events',
    icon: Star,
    description:
      'Thank returning clients and collect a review / testimonial while the event is fresh.',
    steps: [
  ...
  {
    key: 'win-back',
    name: 'Win-Back Dormant Client',
    triggerEvent: 'customer.dormant',
    category: 'events',
    icon: HeartHandshake,
    description:
      'Re-engage past customers who have not booked in a while with a personalized new-location offer.',
    steps: [
      { order: 1, type: 'email', detail: '"We miss you" + trending locations' },
  ...
  // ------------------------- App / system automations -------------------------
  {
    ...
    icon: CreditCard,
    description:
      'When a payment is rejected or fails, prompt the client to retry and alert the finance desk.',
    steps: [
  ...
  },
  {
    ...
    icon: Gift,
    description:
      'Automatically send a celebratory events voucher on a customer birthday or anniversary.',
    steps: [
