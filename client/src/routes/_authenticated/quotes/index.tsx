...
export interface EditableRunOfShowDay {
  id: string;
  day_number: number;
  title: string;
  description: string;
...
// Theme gallery data — Event-Native Proposal Aesthetics
const THEME_OPTIONS = [
  {
    id: 'v12',
    name: 'Royal Wedding Gold',
    desc: 'Rich champagne & royal gold styling',
    colors: ['#2E352B', '#D6A848', '#F1EBD8'],
  },
  {
    id: 'v8',
    name: 'Luxe Noir & Velvet',
    desc: 'Black-tie evening gala & grand reception',
    colors: ['#0F1117', '#D4AF37', '#F8F9FA'],
  },
  {
    id: 'v9',
    name: 'Pastel Blossom',
    desc: 'Blush floral & garden wedding romance',
    colors: ['#881337', '#FDA4AF', '#FFF8F9'],
  },
  {
    id: 'v7',
    name: 'Modern Minimalist',
    desc: 'Clean contemporary white & sage',
    colors: ['#FFFFFF', '#059669', '#1F2937'],
  },
  {
    id: 'v10',
    name: 'Sangeet & Cocktail Glow',
    desc: 'Vibrant jewel tones & festive energy',
    colors: ['#0C4A3E', '#14B8A6', '#F0FDFA'],
  },
  {
    id: 'v13',
    name: 'Corporate Executive',
    desc: 'Refined summit & keynote branding',
...
function QuotesPage() {
  ...
  const [customRunOfShow, setCustomRunOfShow] = useState<EditableRunOfShowDay[]>([]);
  ...
  const handleAiAutoQuote = async () => {
    ...
    try {
      ...
      if (result.customRunOfShow?.length > 0) {
        setCustomRunOfShow(
          result.customRunOfShow.map((d: any) => ({
    ...
    async function loadDetail() {
      ...
      try {
        ...
        if (detail) {
          ...
          // Seed custom runOfShow from base package
          if (detail.runOfShow) {
            setCustomRunOfShow(
              detail.runOfShow.map((d) => ({
                id: Math.random().toString(36).substr(2, 9),
                day_number: d.day_number,
                title: d.title || '',
                description: d.description || '',
  ...
  function autoDetectTheme(themes: string[]) {
    ...
    if (lowerThemes.some((theme) => theme.includes('wedding') || theme.includes('social')))
      setSelectedTheme('v9');
    else if (
      lowerThemes.some((theme) => theme.includes('corporate') || theme.includes('conference'))
    )
      setSelectedTheme('v6');
    else if (lowerThemes.some((theme) => theme.includes('party') || theme.includes('birthday')))
      setSelectedTheme('v10');
    else if (
      lowerThemes.some((theme) => theme.includes('concert') || theme.includes('entertainment'))
    )
      setSelectedTheme('v8');
  ...
  // UI Handlers
  const addActivity = () => {
    if (!pkgDetail?.runOfShow || pkgDetail.runOfShow.length === 0) {
      toast.error('No runOfShow days available to attach activity to.');
      return;
    }
    const newAct: CustomActivity = {
      id: Math.random().toString(36).substr(2, 9),
      dayNumber: pkgDetail.runOfShow[0].day_number,
      name: '',
      price: 0,
    };
  ...
  const updateRunOfShowDay = (
    id: string,
    field: keyof EditableRunOfShowDay,
    value: string | number,
  ) => {
    setCustomRunOfShow((itin) =>
      itin.map((day) => (day.id === id ? { ...day, [field]: value } : day)),
    );
  };

  const removeRunOfShowDay = (id: string) => {
    setCustomRunOfShow((itin) =>
      itin.filter((day) => day.id !== id).map((day, idx) => ({ ...day, day_number: idx + 1 })),
    );
  };

  const addRunOfShowDay = () => {
    const newDay: EditableRunOfShowDay = {
      id: Math.random().toString(36).substr(2, 9),
      day_number: customRunOfShow.length + 1,
      title: 'New Day',
      description: '',
    };
    setCustomRunOfShow([...customRunOfShow, newDay]);
  };
  ...
  const addTransfer = () =>
    setCustomTransfers([
      ...customTransfers,
      {
        id: Math.random().toString(36).substr(2, 9),
        vehicleType: 'SUV',
        serviceType: 'EventSite Transfer',
        pax: 2,
      },
    ]);
  ...
  const pdfProps: QuotePDFProps = useMemo(
    () => ({
      ...
      leadLocation: lead?.location || pkg?.country || 'Location',
  ...
  return (
    <div className="flex flex-col gap-6 pb-12 animate-fade-in">
      ...
      <div className="grid gap-6 lg:grid-cols-[480px_1fr] items-start">
        ...
        <div className="flex flex-col gap-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground truncate">
                        {lead.location || 'No location'} · {lead.budget_range || 'No budget'} ·{' '}
                        {lead.theme || 'General'}
                      </p>
                    </div>
                ...
                activities.map((act) => (
                  ...
                  >
                    ...
                    <Input
                      ...
                      placeholder="e.g. Helicopter Showcase"
                      value={act.name}
          ...
          <Card className="border-border/50 shadow-sm">
            ...
            <CardContent className="space-y-3">
              {customRunOfShow.length === 0 ? (
                <div className="text-xs text-muted-foreground italic text-center py-3 border border-dashed rounded-md">
                  No runOfShow available to customize.
                </div>
              ) : (
                <div className="space-y-3">
                  {customRunOfShow.map((day) => (
                    <div
                      key={day.id}
                    ...
                    >
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <Button
                          ...
                          onClick={() => removeRunOfShowDay(day.id)}
                        >
                          <Trash2 className="w-3.5 h-3.5" />
                        </Button>
                      </div>
                      <Input
                        ...
                        value={day.title}
                        onChange={(e) => updateRunOfShowDay(day.id, 'title', e.target.value)}
                      />
                      <Textarea
                        className="min-h-16 text-xs resize-y bg-background border-border/50 p-2 leading-relaxed"
                        placeholder="Describe the day's activities..."
                        value={day.description}
                        onChange={(e) => updateRunOfShowDay(day.id, 'description', e.target.value)}
                      />
                    </div>
                  ))}
                </div>
              ...
              {customTransfers.length === 0 ? (
                ...
                customTransfers.map((tf) => (
                  ...
                  >
                    ...
                    <Input
                      className="h-7 text-xs"
                      placeholder="Service (e.g. EventSite Pickup)"
        ...
        <div
          style={{
            height: `${(3 + (customRunOfShow.length > 0 ? 1 : 0) + (customStays.length > 0 || customTransfers.length > 0 ? 1 : 0)) * 1170}px`,
          }}
