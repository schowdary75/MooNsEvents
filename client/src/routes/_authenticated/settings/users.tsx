...
const eventBadges = [
  { key: 'credential_elite', label: 'Credential Elite', tone: 'from-sky-500 to-indigo-600' },
...
function EventBadgeSvg({ badgeKey, size = 42 }: { badgeKey?: string | null; size?: number }) {
  const badge = eventBadges.find((item) => item.key === badgeKey) || eventBadges[0];
  ...
  return (
    <div className={`grid gap-2 ${compact ? 'grid-cols-1' : 'md:grid-cols-5'}`}>
      {eventBadges.map((badge) => {
        const selected = (value || 'credential_elite') === badge.key;
        return (
          <button
            key={badge.key}
            type="button"
          ...
          >
            <EventBadgeSvg badgeKey={badge.key} size={compact ? 34 : 46} />
            <div className="min-w-0">
              <div className="text-sm font-semibold">{badge.label}</div>
              <div className="text-[11px] text-muted-foreground">
                {selected ? 'Selected badge' : 'Events profile badge'}
              </div>
            </div>
          </button>
        );
    ...
    try {
      await upsertCrmEmployee({
        data: {
          ...
          badgeKey: employee.badge_key || 'credential_elite',
        },
      });
                ...
                {loading ? (
                  ...
                  employees.map((employee) => (
                    <TableRow key={employee.id}>
                      <TableCell>
                        <div className="flex items-center gap-2">
                          <EventBadgeSvg badgeKey={employee.badge_key} />
                          <div className="grid gap-1">
                            <div className="text-xs font-semibold">
                              {eventBadges.find((badge) => badge.key === employee.badge_key)
                                ?.label || 'Credential Elite'}
                            </div>
                              ...
                              {eventBadges.map((badge) => (
                                <button
                                  ...
                                  className={`h-2.5 w-6 rounded-full bg-gradient-to-r ${badge.tone} ${
                                    (employee.badge_key || 'credential_elite') === badge.key
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          {eventBadges.map((badge) => (
            <div key={badge.key} className="rounded-md border bg-background p-5 text-center">
              <EventBadgeSvg badgeKey={badge.key} size={72} />
              <h3 className="mt-3 font-semibold">{badge.label}</h3>
              <p className="mt-1 text-xs text-muted-foreground">
                Premium events profile badge shown in the sidebar while the employee is logged in.
              </p>
            </div>
          ))}
        </div>
