...
import { PermitCMSPage } from './permit';
import {
...
export const Route = createFileRoute('/_authenticated/content/eventhub')({
  component: EventHubCmsPage,
});

type AdminAuth = { email: string; sessionToken: string };
type PackingCategoryKey = PackingCmsPage['categories'][number]['category_key'];

function EventHubCmsPage() {
  return (
    <div className="space-y-6">
      <div />
      <Tabs defaultValue="permit" className="space-y-5">
        <TabsList>
          <TabsTrigger value="permit">Credential Permit Hub</TabsTrigger>
...
function PackingCmsEditor() {
  ...
  useEffect(() => {
    setRulesPage(1);
  }, [itemSearch, categoryFilter, locationFilter, seasonFilter, activeFilter]);
  ...
  const locationOptions = Array.from(
    new Set(page.items.map((item) => item.location_key).filter(Boolean) as string[]),
  ).sort();
  const seasonOptions = page.seasons.filter((season) => season.is_active);
  const activeItems = page.items.filter((item) => item.is_active);
  const targetedItems = activeItems.filter((item) => item.location_key || item.season_key);
  const filteredItems = page.items
    .map((item, originalIndex) => ({ item, originalIndex }))
    .filter(({ item }) => {
      const matchesSearch =
        !itemSearch.trim() ||
        `${item.item_key} ${item.item_text} ${item.location_key || ''} ${item.season_key || ''}`
          .toLowerCase()
    ...
    <div className="space-y-6">
      <div className="grid gap-3 md:grid-cols-4">
        ...
        <StatCard
          ...
          helper="Location or season scoped"
        />
        <StatCard
          label="Locations"
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Use blank location/season for global items; fill one or both for targeted rules.
          </p>
        </div>
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="grid gap-3 rounded-xl border bg-muted/20 p-3 md:grid-cols-[2fr_1fr_1fr_1fr_130px]">
            <label className="space-y-1 text-sm">
              ...
              <div className="relative">
                ...
                <Input
                  className="pl-9"
                  placeholder="Search item, key, location..."
                  value={itemSearch}
          ...
          <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
            ...
            <Badge variant="outline">Blank location = global</Badge>
            ...
            >
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex flex-wrap gap-2">
                  <Badge variant={item.location_key ? 'default' : 'outline'}>
                    {item.location_key || 'Global'}
                  </Badge>
              ...
              <div className="grid gap-3 md:grid-cols-[1fr_1fr_90px_120px]">
                <SelectField
                  ...
                  onChange={(value) =>
                    updateItem(originalIndex, 'location_key', value.trim() || null)
                  }
                  options={[
                    { value: '', label: 'Global' },
                    ...locationOptions.map((location) => ({
