...
const MODES: Array<{
  ...
  },
  {
    id: 'support',
    label: 'Support',
    description: 'Resolve booking, vendor, refund, and event issues.',
    icon: LifeBuoy,
...
const QUICK_QUESTIONS: Record<SoloOpsMode, string[]> = {
  sales: [
    'Lead found a cheaper quote online for the exact same venue. How to handle this?',
    'A client wants a premium Hyderabad event but the budget is too low. How do I adjust scope?',
    'Lead has gone silent after receiving the quote. Draft a re-engagement message.',
  ],
  product: [
    'Which inclusions should a premium conference package highlight to justify the price?',
    'Client is eventing with elderly parents and wants a relaxed runOfShow. What do I change?',
    'Turn our top FAQs into package page copy.',
  ],
  support: [
    "Customer's permit got rejected and they want a full refund 5 days before events.",
    "Vendor is unresponsive about tomorrow's eventSite transfer. What do I tell the customer?",
    "Guest says the venue room doesn't match photos. Draft the response and action plan.",
  ],
  marketing: [
    'What should I post today to generate high-intent luxury package enquiries?',
    'Draft a WhatsApp broadcast for our Telangana monsoon event offer.',
...
function CommandCenter() {
  ...
  const filteredPackages = useMemo(() => {
    ...
    return rows
      .filter((pkg) =>
        [pkg.name, pkg.location, pkg.country, pkg.category, ...pkg.themes].some((item) =>
  ...
  function injectLeadContext(lead: {
    ...
    setQuestion((current) => {
      const context = `Context — lead: ${lead.name}, location: ${lead.location || 'open'}, budget: ${lead.budget_range || 'unknown'}, status: ${lead.status}.\n\n`;
      const seventped = current.replace(/^Context — lead:.*\n\n/, '');
      return context + seventped;
    });
              ...
              {(brief?.leads || []).slice(0, 5).map((lead) => (
                ...
                >
                  ...
                  <p className="mt-0.5 truncate text-[11px] text-muted-foreground">
                    {lead.location || 'Open location'} · {lead.budget_range || 'Budget TBD'}
                  </p>
                </button>
              ))}
                ...
                <div key={pkg.id} className="rounded-lg border border-border/40 bg-muted/10 p-2.5">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      ...
                      <p className="text-[11px] text-muted-foreground">
                        {pkg.location} · {pkg.days}D/{pkg.nights}N · {money(pkg.price)}
                      </p>
                    </div>
                    <Badge
