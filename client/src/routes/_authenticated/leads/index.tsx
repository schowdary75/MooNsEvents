...
import { LeadWhatsAppModal } from '@/components/whatsapp/LeadWhatsAppModal';
import { WhatsAppIcon } from '@/components/whatsapp/WhatsAppIcon';
import { getWhatsAppUnreadCounts } from '@/api/whatsappClient';
import { LeadEventScopeCard } from '@/components/LeadEventScopeCard';
import {
  Table,
  TableBody,
  TableCell,
...
function LeadsPage() {
  ...
  const [whatsappLead, setWhatsappLead] = useState<LeadSubmissionRow | null>(null);
  const [whatsappOpen, setWhatsappOpen] = useState(false);
  const [whatsappUnread, setWhatsappUnread] = useState<Record<string, number>>({});
    ...
    try {
      const res = await adminAiAnalyzeLeadPriority({
        data: {
          ...
          inquiryMessage: selected.notes || selected.location || 'No message provided',
        },
      });
  ...
  async function load() {
    ...
    setLoading(true);
    try {
      const [board, inquiryRows, unreadCounts] = await Promise.all([
        adminGetLeadCrmBoard({ data: { auth } }),
        adminGetInquiries({ data: { auth } }),
        getWhatsAppUnreadCounts().catch(() => ({})),
      ]);
      ...
      setWhatsappUnread(unreadCounts);
  ...
  function openWhatsApp(lead: LeadSubmissionRow) {
  ...
  return (
    <div className="space-y-5 animate-fade-in">
        ...
        <div className="glass-card rounded-xl p-5 animate-scale-in">
          ...
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <Input
              ...
              value={leadForm.eventMonth}
              onChange={(e) => setLeadForm({ ...leadForm, eventMonth: e.target.value })}
              className="border-border/50"
            />
            <Input
              type="number"
              placeholder="Clients"
              value={leadForm.clientsCount}
              onChange={(e) => setLeadForm({ ...leadForm, clientsCount: Number(e.target.value) })}
              className="border-border/50"
            />
            <Input
          ...
          <div className="flex flex-col gap-2 glass-card rounded-xl p-3 md:flex-row md:items-center">
            <div className="relative flex-1">
              ...
              <Input
                ...
                placeholder="Search name, location, phone, source..."
                value={query}
                ...
                {loading ? (
                  ...
                  paginatedItems.map((lead, idx) => (
                    ...
                    >
                      ...
                      <TableCell>
                        <div className="text-sm font-medium">{lead.location || 'Open'}</div>
                        <div className="text-[11px] text-muted-foreground">
                          {lead.event_month || 'Flexible'} · {lead.clients_count || 2} pax ·{' '}
                      ...
                      >
                        <Button
                          ...
                          variant="ghost"
                          size="icon"
                          className="relative h-8 w-8 rounded-full text-emerald-600 hover:bg-emerald-500/10 hover:text-emerald-700"
                          onClick={() => openWhatsApp(lead)}
                          aria-label={`Open WhatsApp conversation with ${lead.name}`}
                          title={`WhatsApp ${lead.name}`}
                        >
                          <WhatsAppIcon className="h-5 w-5" />
                          {(whatsappUnread[String(lead.id)] || 0) > 0 ? (
                            <span
                              className="absolute -right-1 -top-1 flex min-h-4 min-w-4 items-center justify-center rounded-full bg-rose-600 px-1 text-[9px] font-bold text-white"
                              aria-label={`${whatsappUnread[String(lead.id)]} unread WhatsApp messages`}
                            >
                              {Math.min(99, whatsappUnread[String(lead.id)])}
                            </span>
                          ) : null}
                        </Button>
                      </TableCell>
              ...
              <div className="relative px-5 pt-5 pb-4 border-b border-border/30">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">
                        {selected.location || 'Open location'} ·{' '}
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-8 gap-1.5 border-emerald-500/40 text-xs text-emerald-700 hover:bg-emerald-500/10 dark:text-emerald-400"
                ...
                >
                  <div>
                    <div className="font-semibold">
                      {callback.name} · {callback.location || 'General'}
                    </div>
      ...
      <LeadWhatsAppModal
        ...
        agentName={user?.name || user?.email || 'MooNs agent'}
        canConfigure={user?.role === 'admin' && Boolean(user?.platformUserId)}
        onUnreadChange={() =>
          void getWhatsAppUnreadCounts()
            .then(setWhatsappUnread)
            .catch(() => undefined)
        }
      />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 md:p-8">
          <div className="bg-card w-full max-w-6xl h-full max-h-[90vh] rounded-2xl shadow-2xl flex flex-col overflow-hidden relative animate-scale-in">
            ...
            <div className="flex-1 w-full bg-[#f3f4f6]">
              <iframe
                src={`/banners.html?status=${encodeURIComponent(getBannerStatus(selected.status))}&name=${encodeURIComponent(selected.name || '')}&phone=${encodeURIComponent(selected.phone || '')}&dest=${encodeURIComponent(selected.location || '')}`}
