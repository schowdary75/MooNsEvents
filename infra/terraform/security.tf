...
resource "aws_cloudwatch_event_api_location" "malware_webhook" {
  name                             = "${local.name}-malware-webhook"
...
resource "aws_iam_role" "eventbridge_location" {
  name = "${local.name}-eventbridge-location"
  assume_role_policy = jsonencode({
    Version   = "2012-10-17"
...
resource "aws_iam_role_policy" "eventbridge_location" {
  role = aws_iam_role.eventbridge_location.id
  policy = jsonencode({
    Version   = "2012-10-17"
    Statement = [{ Effect = "Allow", Action = "events:InvokeApiLocation", Resource = aws_cloudwatch_event_api_location.malware_webhook.arn }]
  })
}

resource "aws_cloudwatch_event_target" "malware_results" {
  rule     = aws_cloudwatch_event_rule.malware_results.name
  arn      = aws_cloudwatch_event_api_location.malware_webhook.arn
  role_arn = aws_iam_role.eventbridge_location.arn
}
